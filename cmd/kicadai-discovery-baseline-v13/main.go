...
func run(parent context.Context, arguments []string, stdout io.Writer) error {
	flags := flag.NewFlagSet("kicadai-discovery-baseline-v13", flag.ContinueOnError)
	...
	flags.StringVar(&opts.workingRoot, "working-root", "", "fresh V13 root for per-case replay and promotion evidence")
	flags.StringVar(&opts.reportPath, "report", "", "fresh V13 generation-zero report path")
	flags.StringVar(&opts.evaluatorManifest, "evaluator-manifest", "", "frozen V13 evaluator checksum manifest")
	...
	flags.BoolVar(&opts.resume, "resume", false, "resume authenticated V13 completed-case checkpoints")
	if err := flags.Parse(arguments); err != nil {
		return fmt.Errorf("parse V13 discovery evaluator arguments: %s", strings.TrimSpace(diagnostics.String()))
	}
	if flags.NArg() != 0 {
		return fmt.Errorf("parse V13 discovery evaluator arguments: unexpected positional arguments")
	}
	...
	if err != nil {
		return fmt.Errorf("hash V13 evaluator manifest: %w", err)
	}
	...
	report, err := capabilityexecutorv10.New().RunV13(ctx, capabilityexecutorv10.Request{
	...
	return json.NewEncoder(stdout).Encode(summary{
		Schema: "kicadai.closed-loop-open-set-discovery-baseline-run.v13", Version: 13,
...
func normalizeOptions(opts *options) error {
	...
	defaults := map[*string]string{
		...
		&opts.evaluatorManifest: filepath.Join(root, "specs", "closed-loop-open-set-capability-expansion", "V13_EVALUATOR.sha256"),
...
func ensureCleanRepository(ctx context.Context, root string) error {
	...
	if err != nil {
		return fmt.Errorf("V13 evaluation requires the repository root")
	}
	...
	if len(status) != 0 {
		return fmt.Errorf("V13 evaluation requires a clean committed tree")
	}
	return nil
}
...
func writeAtomicNoReplace(path string, data []byte, trailingNewline bool) error {
	...
	temporary, err := os.CreateTemp(filepath.Dir(path), ".v13-baseline-*.tmp")
	if err != nil {
		return fmt.Errorf("create temporary V13 discovery report: %w", err)
	}
	...
	if err := os.Link(temporaryPath, path); err != nil {
		return fmt.Errorf("atomically publish V13 discovery report without replacement: %w", err)
	}
	return nil
}
