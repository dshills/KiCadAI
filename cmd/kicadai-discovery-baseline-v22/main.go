...
const evaluatorVersion = 22
const selectedPopulationSHA256 = "556cbd753280c321b7d7466d01f57c1a4e3482a7133eac381cc2ecab5c1edf52"
...
func run(parent context.Context, arguments []string, stdout io.Writer) error {
	flags := flag.NewFlagSet("kicadai-discovery-baseline-v22", flag.ContinueOnError)
	...
	flags.StringVar(&opts.workingRoot, "working-root", "", "fresh V22 root for per-case replay and promotion evidence")
	flags.StringVar(&opts.reportPath, "report", "", "fresh V22 generation-zero report path")
	flags.StringVar(&opts.evaluatorManifest, "evaluator-manifest", "", "frozen V22 evaluator checksum manifest")
	flags.StringVar(&opts.selectedPopulation, "selected-population", "", "frozen V22 selected public topology population")
	...
	if err := flags.Parse(arguments); err != nil {
		return fmt.Errorf("parse V22 discovery evaluator arguments: %s", strings.TrimSpace(diagnostics.String()))
	}
	if flags.NArg() != 0 {
		return fmt.Errorf("parse V22 discovery evaluator arguments: unexpected positional arguments")
	}
	...
	selectedCaseIDs, err := loadSelectedCaseIDs(opts.selectedPopulation, selectedPopulationSHA256, "kicadai.public-electrical-population.v22", 22, selectedPopulationCaseCount)
	if err != nil {
		return err
	}
	...
	if err != nil {
		return fmt.Errorf("hash V22 evaluator manifest: %w", err)
	}
	...
	// V18, V20, V21 and V22 authenticate the same immutable V18 catalog, registry,
	...
	executor, err := capabilityexecutorv10.NewSelectedV22WithLegacy(
	...
	report, err := executor.RunV22(ctx, capabilityexecutorv10.Request{
	...
	return json.NewEncoder(stdout).Encode(summary{
		Schema: "kicadai.closed-loop-open-set-discovery-baseline-run.v22", Version: evaluatorVersion,
...
func loadSelectedCaseIDs(path, expectedHash, schema string, version, count int) ([]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("open V22 selected public topology population: %w", err)
	}
	...
	if err != nil || pathErr != nil || !info.Mode().IsRegular() || !pathInfo.Mode().IsRegular() || !os.SameFile(info, pathInfo) || info.Size() <= 0 || info.Size() > 1<<20 {
		return nil, fmt.Errorf("V22 selected public topology population is not a bounded nonempty regular file")
	}
	data, err := io.ReadAll(io.LimitReader(file, 1<<20+1))
	if err != nil {
		return nil, fmt.Errorf("read V22 selected public topology population: %w", err)
	}
	if int64(len(data)) != info.Size() {
		return nil, fmt.Errorf("V22 selected public topology population changed while reading")
	}
	digest := sha256.Sum256(data)
	if fmt.Sprintf("%x", digest) != expectedHash {
		return nil, fmt.Errorf("V22 selected public topology population differs from its frozen commitment")
	}
	...
	if err := json.Unmarshal(data, &population); err != nil {
		return nil, fmt.Errorf("decode V22 selected public topology population: %w", err)
	}
	if population.Schema != schema || population.Version != version || len(population.SelectedCases) != count {
		return nil, fmt.Errorf("V22 selected public topology population identity or size is invalid")
	}
	...
	for _, selected := range population.SelectedCases {
		if strings.TrimSpace(selected.ID) == "" || seen[selected.ID] {
			return nil, fmt.Errorf("V22 selected public topology population contains an empty or duplicate identity")
		}
...
func normalizeOptions(opts *options) error {
	...
	defaults := map[*string]string{
		...
		&opts.evaluatorManifest:  filepath.Join(root, "specs", "post-topology-electrical-blockers", "public-evaluation-v22", "EVALUATOR.sha256"),
		&opts.selectedPopulation: filepath.Join(root, "specs", "post-topology-electrical-blockers", "V22_PUBLIC_POPULATION.json"),
...
func ensureCleanRepository(ctx context.Context, root string) error {
	...
	if err != nil {
		return fmt.Errorf("V22 evaluation requires git to verify its clean committed source tree: %w", err)
	}
	topLevel, err := exec.CommandContext(ctx, gitPath, "-C", root, "rev-parse", "--show-toplevel").Output()
	if err != nil {
		return fmt.Errorf("resolve V22 repository root with git: %w", err)
	}
	...
	if rootErr != nil || topErr != nil || !rootInfo.IsDir() || !topInfo.IsDir() || !os.SameFile(rootInfo, topInfo) {
		return fmt.Errorf("V22 evaluation root is not the Git top-level directory")
	}
	...
	if len(status) != 0 {
		return fmt.Errorf("V22 evaluation requires a clean committed tree")
	}
	return nil
}
...
func writeAtomicNoReplace(path string, data []byte, trailingNewline bool) error {
	...
	temporary, err := os.CreateTemp(filepath.Dir(path), ".v22-baseline-*.tmp")
	if err != nil {
		return fmt.Errorf("create temporary V22 discovery report: %w", err)
	}
	...
	if err := os.Link(temporaryPath, path); err != nil {
		return fmt.Errorf("atomically publish V22 discovery report without replacement: %w", err)
	}
	return nil
}
