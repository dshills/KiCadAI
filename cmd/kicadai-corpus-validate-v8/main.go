...
func run(arguments []string, stdout io.Writer) error {
	flags := flag.NewFlagSet("kicadai-corpus-validate-v8", flag.ContinueOnError)
	flags.SetOutput(io.Discard)
	packetRoot := flags.String("packet-root", "", "frozen V8 public authoring packet root")
	historyPath := flags.String("history", "", "sanitized V1-V7 commitment JSON")
	outputPath := flags.String("output", "", "aggregate V8 validation report JSON")
	...
	if historical.Base.SourceSHA256 != policy.HistoricalCommitmentsSHA256 {
		return fmt.Errorf("historical commitment source does not match frozen V8 policy")
	}
	...
	if err := corpusfreezev8.WriteReport(*outputPath, report); err != nil {
		return err
	}
	_, err = fmt.Fprintf(stdout, "validated %d V8 behavior cases across %d isolated authors\n", len(report.Entries), len(policy.AuthorSlots))
	return err
}
...
func usageError() error {
	return fmt.Errorf("usage: kicadai-corpus-validate-v8 -packet-root PATH -history PATH -bundle author_1=PATH ... -bundle author_6=PATH -output PATH")
}
