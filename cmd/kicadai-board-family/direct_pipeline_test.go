...
// These are synthetic transport fixtures, never captured model outputs.
func directCorpusFixture(t testing.TB, id, prompt string) []byte {
	t.Helper()
	var wire struct{ Facts []json.RawMessage }
	if err := json.Unmarshal(connectionCorpusFixture(t, id, prompt), &wire); err != nil {
		t.Fatal(err)
	}
	b, err := json.Marshal(map[string]any{"version": boardfamily.DirectEvidenceVersion, "facts": wire.Facts})
	if err != nil {
		t.Fatal(err)
	}
	return b
...
func TestDirectCommandSyntheticCorpus(t *testing.T) {
	...
	for _, c := range spec.Cases {
		t.Run(c.ID, func(t *testing.T) {
			raw := directCorpusFixture(t, c.ID, c.Prompt)
			before := bytes.Clone(raw)
			decision, err := boardfamily.DecodeDirectEvidenceIntent(c.Prompt, raw)
			...
			if err := save(budget, boardfamily.LedgerPolicy{Goal: "offline-direct-" + c.ID, MaxRequests: 1, MaxMicroUSD: 50000}); err != nil {
				t.Fatal(err)
			}
			t.Setenv("OPENAI_API_KEY", "offline-command-placeholder")
			p, counts := directCommandPipeline(t, raw, "", "")
			if _, err := runIndexedCommand(t, p, "--intent-protocol", "direct-v6", "--prompt", c.Prompt, "--live-budget", budget, "--ledger", ledger, "--evidence-journal", journal, "--output", out); err != nil {
				t.Fatal(err)
			}
			want := 0
			...
			a, err := boardfamily.InspectDirectJournal(journal)
			if err != nil || !reflect.DeepEqual(a.Selection.Decision, decision) || a.Selection.AdmissionVersion != boardfamily.DirectEvidenceVersion || len(a.FilesSHA256) != 8 || len(a.Ledger.Entries) != 1 {
			...
			for _, inspect := range []func(string) (boardfamily.ReferencedJournalAudit, error){boardfamily.InspectConnectionJournal, boardfamily.InspectOwnedJournal, boardfamily.InspectReferencedJournal} {
			...
			if _, err := runIndexedCommand(t, p, "--inspect-direct-journal", journal); err != nil || counts.requests != 1 {
...
func TestDirectCommandFailureGates(t *testing.T) {
	for _, mode := range []string{"invalid-extraction", "malformed-output", "refusal", "server-error", "missing-model", "broken-stream", "generation-failure", "validation-failure"} {
		t.Run(mode, func(t *testing.T) {
			...
			raw := directCorpusFixture(t, "useful-01", prompt)
			...
			root := t.TempDir()
			out, journal, ledger, budget := filepath.Join(root, "out"), filepath.Join(root, "journal"), filepath.Join(root, "ledger.json"), filepath.Join(root, "budget.json")
			if err := save(budget, boardfamily.LedgerPolicy{Goal: "offline-direct-failure", MaxRequests: 1, MaxMicroUSD: 50000}); err != nil {
				t.Fatal(err)
			}
			p, counts := directCommandPipeline(t, raw, mode, "")
			_, err := runIndexedCommand(t, p, "--intent-protocol", "direct-v6", "--prompt", prompt, "--live-budget", budget, "--ledger", ledger, "--evidence-journal", journal, "--output", out)
...
func TestDirectFlagsAndContract(t *testing.T) {
	for _, mode := range []string{"export", "export-file", "missing-prompt", "both-prompts", "no-journal", "no-budget", "no-ledger", "overlap", "mixed-inspect", "inspect-generation", "config", "nil-selector"} {
		t.Run(mode, func(t *testing.T) {
			...
			p, counts := directCommandPipeline(t, []byte(`{}`), "", "")
			args := []string{"--intent-protocol", "direct-v6", "--prompt", prompt, "--output", out, "--ledger", ledger, "--live-budget", budget, "--evidence-journal", journal}
			switch mode {
			case "export", "export-file", "missing-prompt", "both-prompts":
				args = []string{"--intent-protocol", "direct-v6", "--export-live-contract", out}
			...
			case "mixed-inspect":
				args = []string{"--inspect-direct-journal", journal, "--inspect-owned-journal", journal}
			case "inspect-generation":
				args = append(args, "--inspect-direct-journal", journal)
			case "config":
				args = append(args, "--config", "never-read.json")
			case "nil-selector":
				p.interpretDirect = nil
			}
			...
			want, err := boardfamily.DirectEvidenceContract(prompt)
			if err != nil {
				t.Fatal(err)
			}
			var got any
