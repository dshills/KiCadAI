...
func run(arguments []string, stdout io.Writer) error {
	flags := flag.NewFlagSet("kicadai-corpus-validate-v6", flag.ContinueOnError)
	flags.SetOutput(io.Discard)
	packetRoot := flags.String("packet-root", "", "frozen V6 public authoring packet root")
	historyPath := flags.String("history", "", "sanitized V1-V5 commitment JSON")
	outputPath := flags.String("output", "", "aggregate V6 validation report JSON")
	...
	_, err = fmt.Fprintf(stdout, "validated %d V6 behavior cases across %d isolated authors\n", len(report.Entries), len(policy.AuthorSlots))
	return err
}
...
func usageError() error {
	return fmt.Errorf("usage: kicadai-corpus-validate-v6 -packet-root PATH -history PATH -bundle author_1=PATH -bundle author_2=PATH -bundle author_3=PATH -output PATH")
}
