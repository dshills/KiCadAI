...
func TestRunHelpSucceeds(t *testing.T) {
	...
	if !strings.Contains(stdout.String(), "Usage of kicadai-corpus-publish-v7:") ||
		!strings.Contains(stdout.String(), "-freeze-parent-commit") {
		t.Fatalf("help output = %q", stdout.String())
	}
}

func TestRunFlagErrorDoesNotWriteStdout(t *testing.T) {
	var stdout bytes.Buffer
	if err := run([]string{"-unknown"}, &stdout); err == nil || !strings.Contains(err.Error(), "flag provided but not defined") {
		t.Fatalf("unknown flag error = %v", err)
	}
	if stdout.Len() != 0 {
		t.Fatalf("flag error wrote stdout: %q", stdout.String())
	}
}
...
func TestVerifyFrozenCommitsRejectsSubstitution(t *testing.T) {
	if err := verifyFrozenCommits(v7StartingCommit, v7ContractFreezeCommit, v7AuthorPacketCommit, v7ValidatorCommit); err != nil {
		t.Fatal(err)
	}
	if err := verifyFrozenCommits(v7StartingCommit, v7ContractFreezeCommit, v7AuthorPacketCommit, strings.Repeat("0", 40)); err == nil || !strings.Contains(err.Error(), "validator commit") {
		t.Fatalf("validator substitution error = %v", err)
	}
}
