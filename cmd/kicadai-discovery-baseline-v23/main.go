...
const evaluatorVersion = 23
const selectedPopulationSHA256 = "b5aef7c5b39facab162928e6feb390bb9c78883064eb984eeffceadde5cd4a5c"
const selectedPopulationCaseCount = 4
...
func run(parent context.Context, arguments []string, stdout io.Writer) error {
	flags := flag.NewFlagSet("kicadai-discovery-baseline-v23", flag.ContinueOnError)
	...
	flags.StringVar(&opts.workingRoot, "working-root", "", "fresh V23 root for per-case replay and promotion evidence")
	flags.StringVar(&opts.reportPath, "report", "", "fresh V23 generation-zero report path")
	flags.StringVar(&opts.evaluatorManifest, "evaluator-manifest", "", "frozen V23 evaluator checksum manifest")
	flags.StringVar(&opts.selectedPopulation, "selected-population", "", "frozen V23 selected public topology population")
	...
	flags.StringVar(&opts.v21Population, "v21-population", "", "immutable selected V21 predecessor population")
	if err := flags.Parse(arguments); err != nil {
		return fmt.Errorf("parse V23 discovery evaluator arguments: %s", strings.TrimSpace(diagnostics.String()))
	}
	if flags.NArg() != 0 {
		return fmt.Errorf("parse V23 discovery evaluator arguments: unexpected positional arguments")
	}
	...
	selectedCaseIDs, err := loadSelectedCaseIDs(opts.selectedPopulation, selectedPopulationSHA256, "kicadai.public-electrical-population.v23", 23, selectedPopulationCaseCount)
	if err != nil {
		return err
	}
	v21SelectedIDs, err := loadSelectedCaseIDs(opts.v21Population, "5809ba88980d3745727f3c0a3324cc987105c94a2a2a816baf54382d25ab70d1", "kicadai.public-causal-topology-population.v21", 21, 8)
	if err != nil {
		return err
	}
	evaluatorManifestSHA256, err := capabilityexecutorv10.RegularFileSHA256(opts.evaluatorManifest)
	if err != nil {
		return fmt.Errorf("hash V23 evaluator manifest: %w", err)
	}
	...
	// V18, V20, V21 and V23 authenticate the same immutable V18 catalog, registry,
	...
	executor, err := capabilityexecutorv10.NewSelectedV23WithLegacy(
		corpus.Cases, v21SelectedIDs, selectedCaseIDs,
	...
	report, err := executor.RunV23(ctx, capabilityexecutorv10.Request{
	...
	return json.NewEncoder(stdout).Encode(summary{
		Schema: "kicadai.closed-loop-open-set-discovery-baseline-run.v23", Version: evaluatorVersion,
...
func loadSelectedCaseIDs(path, expectedHash, schema string, version, count int) ([]string, error) {
	file, err := os.Open(path)
	if err != nil {
		return nil, fmt.Errorf("open V%d selected public topology population: %w", version, err)
	}
	...
	if err != nil || pathErr != nil || !info.Mode().IsRegular() || !pathInfo.Mode().IsRegular() || !os.SameFile(info, pathInfo) || info.Size() <= 0 || info.Size() > 1<<20 {
		return nil, fmt.Errorf("V%d selected public topology population is not a bounded nonempty regular file", version)
	}
	data, err := io.ReadAll(io.LimitReader(file, 1<<20+1))
	if err != nil {
		return nil, fmt.Errorf("read V%d selected public topology population: %w", version, err)
	}
	if int64(len(data)) != info.Size() {
		return nil, fmt.Errorf("V%d selected public topology population changed while reading", version)
	}
	digest := sha256.Sum256(data)
	if fmt.Sprintf("%x", digest) != expectedHash {
		return nil, fmt.Errorf("V%d selected public topology population differs from its frozen commitment", version)
	}
	...
	if err := json.Unmarshal(data, &population); err != nil {
		return nil, fmt.Errorf("decode V%d selected public topology population: %w", version, err)
	}
	if population.Schema != schema || population.Version != version || len(population.SelectedCases) != count {
		return nil, fmt.Errorf("V%d selected public topology population identity or size is invalid", version)
	}
	...
	for _, selected := range population.SelectedCases {
		if strings.TrimSpace(selected.ID) == "" || seen[selected.ID] {
			return nil, fmt.Errorf("V%d selected public topology population contains an empty or duplicate identity", version)
		}
...
func normalizeOptions(opts *options) error {
	...
	defaults := map[*string]string{
		...
		&opts.evaluatorManifest:  filepath.Join(root, "specs", "electrical-repair-followup-v23", "public-evaluation-v23", "EVALUATOR.sha256"),
		&opts.selectedPopulation: filepath.Join(root, "specs", "electrical-repair-followup-v23", "V23_PUBLIC_POPULATION.json"),
		&opts.v21Population:      filepath.Join(root, "specs", "generic-causal-topology-repair", "V21_PUBLIC_TOPOLOGY_POPULATION.json"),
	...
	for _, target := range []*string{&opts.corpusRoot, &opts.workingRoot, &opts.reportPath, &opts.evaluatorManifest, &opts.selectedPopulation, &opts.v21Population, &opts.toolchainLock} {
...
func ensureCleanRepository(ctx context.Context, root string) error {
	...
	if err != nil {
		return fmt.Errorf("V23 evaluation requires git to verify its clean committed source tree: %w", err)
	}
	topLevel, err := exec.CommandContext(ctx, gitPath, "-C", root, "rev-parse", "--show-toplevel").Output()
	if err != nil {
		return fmt.Errorf("resolve V23 repository root with git: %w", err)
	}
	...
	if rootErr != nil || topErr != nil || !rootInfo.IsDir() || !topInfo.IsDir() || !os.SameFile(rootInfo, topInfo) {
		return fmt.Errorf("V23 evaluation root is not the Git top-level directory")
	}
	...
	if len(status) != 0 {
		return fmt.Errorf("V23 evaluation requires a clean committed tree")
	}
	return nil
}
...
func writeAtomicNoReplace(path string, data []byte, trailingNewline bool) error {
	...
	temporary, err := os.CreateTemp(filepath.Dir(path), ".v23-baseline-*.tmp")
	if err != nil {
		return fmt.Errorf("create temporary V23 discovery report: %w", err)
	}
	...
	if err := os.Link(temporaryPath, path); err != nil {
		return fmt.Errorf("atomically publish V23 discovery report without replacement: %w", err)
	}
	return nil
}
