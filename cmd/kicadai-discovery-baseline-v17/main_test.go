...
func TestNormalizeOptionsUsesV17ManifestAndRejectsExistingReport(t *testing.T) {
	root := t.TempDir()
	...
	if !strings.HasSuffix(opts.evaluatorManifest, filepath.Join("closed-loop-open-set-capability-expansion", "V17_EVALUATOR.sha256")) {
		t.Fatalf("V17 evaluator manifest = %q", opts.evaluatorManifest)
	}
}
...
func TestWriteAtomicNoReplaceRefusesReplacement(t *testing.T) {
	...
	if err := writeAtomicNoReplace(path, []byte("second"), false); err == nil {
		t.Fatal("V17 report replacement succeeded")
	}
	data, err := os.ReadFile(path)
	if err != nil || string(data) != "first" {
		t.Fatalf("V17 report changed: %q, %v", data, err)
	}
}
