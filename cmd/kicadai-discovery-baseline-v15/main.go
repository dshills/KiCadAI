...
func run(parent context.Context, arguments []string, stdout io.Writer) error {
	flags := flag.NewFlagSet("kicadai-discovery-baseline-v15", flag.ContinueOnError)
	...
	flags.StringVar(&opts.workingRoot, "working-root", "", "fresh V15 root for per-case replay and promotion evidence")
	flags.StringVar(&opts.reportPath, "report", "", "fresh V15 generation-zero report path")
	flags.StringVar(&opts.evaluatorManifest, "evaluator-manifest", "", "frozen V15 evaluator checksum manifest")
	...
	flags.BoolVar(&opts.resume, "resume", false, "resume authenticated V15 completed-case checkpoints")
	if err := flags.Parse(arguments); err != nil {
		return fmt.Errorf("parse V15 discovery evaluator arguments: %s", strings.TrimSpace(diagnostics.String()))
	}
	if flags.NArg() != 0 {
		return fmt.Errorf("parse V15 discovery evaluator arguments: unexpected positional arguments")
	}
	...
	if err != nil {
		return fmt.Errorf("hash V15 evaluator manifest: %w", err)
	}
	...
	report, err := capabilityexecutorv10.NewV15().RunV15(ctx, capabilityexecutorv10.Request{
	...
	return json.NewEncoder(stdout).Encode(summary{
		Schema: "kicadai.closed-loop-open-set-discovery-baseline-run.v15", Version: 15,
...
func normalizeOptions(opts *options) error {
	...
	defaults := map[*string]string{
		...
		&opts.evaluatorManifest: filepath.Join(root, "specs", "closed-loop-open-set-capability-expansion", "V15_EVALUATOR.sha256"),
...
func ensureCleanRepository(ctx context.Context, root string) error {
	...
	if err != nil {
		return fmt.Errorf("resolve V15 repository root with git: %w", err)
	}
	...
	if rootErr != nil || topErr != nil || !rootInfo.IsDir() || !topInfo.IsDir() || !os.SameFile(rootInfo, topInfo) {
		return fmt.Errorf("V15 evaluation root is not the Git top-level directory")
	}
	...
	if len(status) != 0 {
		return fmt.Errorf("V15 evaluation requires a clean committed tree")
	}
	return nil
}
...
func writeAtomicNoReplace(path string, data []byte, trailingNewline bool) error {
	...
	temporary, err := os.CreateTemp(filepath.Dir(path), ".v15-baseline-*.tmp")
	if err != nil {
		return fmt.Errorf("create temporary V15 discovery report: %w", err)
	}
	...
	if err := os.Link(temporaryPath, path); err != nil {
		return fmt.Errorf("atomically publish V15 discovery report without replacement: %w", err)
	}
	return nil
}
