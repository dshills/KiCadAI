...
func run(arguments []string, stdout io.Writer) error {
	flags := flag.NewFlagSet("kicadai-corpus-validate-v10", flag.ContinueOnError)
	flags.SetOutput(io.Discard)
	packetRoot := flags.String("packet-root", "", "frozen V10 public authoring packet root")
	historyPath := flags.String("history", "", "sanitized V1-V8 commitment JSON")
	outputPath := flags.String("output", "", "aggregate V10 validation report JSON")
	...
	if err := corpusfreezev10.WriteReport(*outputPath, report); err != nil {
		return err
	}
	_, err = fmt.Fprintf(stdout, "validated %d V10 behavior cases across %d isolated authors\n", len(report.Entries), len(policy.AuthorSlots))
	return err
}
...
func usageError() error {
	return fmt.Errorf("usage: kicadai-corpus-validate-v10 -packet-root PATH -history PATH -bundle author_1=PATH ... -bundle author_6=PATH -output PATH")
}
