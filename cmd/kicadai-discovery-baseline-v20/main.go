...
func run(parent context.Context, arguments []string, stdout io.Writer) error {
	flags := flag.NewFlagSet("kicadai-discovery-baseline-v20", flag.ContinueOnError)
	...
	flags.StringVar(&opts.workingRoot, "working-root", "", "fresh V20 root for per-case replay and promotion evidence")
	flags.StringVar(&opts.reportPath, "report", "", "fresh V20 generation-zero report path")
	flags.StringVar(&opts.evaluatorManifest, "evaluator-manifest", "", "frozen V20 evaluator checksum manifest")
	...
	flags.BoolVar(&opts.resume, "resume", false, "resume authenticated V20 completed-case checkpoints")
	if err := flags.Parse(arguments); err != nil {
		return fmt.Errorf("parse V20 discovery evaluator arguments: %s", strings.TrimSpace(diagnostics.String()))
	}
	if flags.NArg() != 0 {
		return fmt.Errorf("parse V20 discovery evaluator arguments: unexpected positional arguments")
	}
	...
	if err != nil {
		return fmt.Errorf("hash V20 evaluator manifest: %w", err)
	}
	...
	if len(v18ModelDiagnostics) != 0 {
		messages := make([]string, 0, len(v18ModelDiagnostics))
		for _, diagnostic := range v18ModelDiagnostics {
			messages = append(messages, diagnostic.Message)
		}
		return fmt.Errorf("load exact V18 model provenance: %s", strings.Join(messages, "; "))
	}
	...
	// V20 adds admission and no catalog family. Load a separate V18-backed copy
	// so the V20 and exact V18 environment bindings are explicit even while
	// their authenticated catalog and model-registry hashes remain equal.
	catalog, err := components.LoadCatalogV18(ctx)
	if err != nil {
		return fmt.Errorf("load embedded component catalog: %w", err)
	}
	models, modelDiagnostics := modelprovenance.LoadV18()
	if len(modelDiagnostics) != 0 {
		messages := make([]string, 0, len(modelDiagnostics))
		for _, diagnostic := range modelDiagnostics {
			messages = append(messages, diagnostic.Message)
		}
		return fmt.Errorf("load model provenance: %s", strings.Join(messages, "; "))
	}
	...
	report, err := capabilityexecutorv10.NewV20WithLegacy(v18Inventory, v18Simulation, legacyInventory, legacySimulation).RunV20(ctx, capabilityexecutorv10.Request{
	...
	return json.NewEncoder(stdout).Encode(summary{
		Schema: "kicadai.closed-loop-open-set-discovery-baseline-run.v20", Version: evaluatorVersion,
...
func normalizeOptions(opts *options) error {
	...
	defaults := map[*string]string{
		...
		&opts.evaluatorManifest: filepath.Join(root, "specs", "generic-analysis-model-solver-admission", "V20_EVALUATOR.sha256"),
...
func ensureCleanRepository(ctx context.Context, root string) error {
	...
	if err != nil {
		return fmt.Errorf("resolve V20 repository root with git: %w", err)
	}
	...
	if rootErr != nil || topErr != nil || !rootInfo.IsDir() || !topInfo.IsDir() || !os.SameFile(rootInfo, topInfo) {
		return fmt.Errorf("V20 evaluation root is not the Git top-level directory")
	}
	...
	if len(status) != 0 {
		return fmt.Errorf("V20 evaluation requires a clean committed tree")
	}
	return nil
}
...
func writeAtomicNoReplace(path string, data []byte, trailingNewline bool) error {
	...
	temporary, err := os.CreateTemp(filepath.Dir(path), ".v20-baseline-*.tmp")
	if err != nil {
		return fmt.Errorf("create temporary V20 discovery report: %w", err)
	}
	...
	if err := os.Link(temporaryPath, path); err != nil {
		return fmt.Errorf("atomically publish V20 discovery report without replacement: %w", err)
	}
	return nil
}
