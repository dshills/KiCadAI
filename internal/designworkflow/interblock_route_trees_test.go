...
func TestBuildInterBlockRouteTreePrefersConnectorRoot(t *testing.T) {
	group := routeTreeTestGroup("SIG", InterBlockRouteEndpoint{Ref: "U1", Pin: "1"}, InterBlockRouteEndpoint{Ref: "J1", Pin: "1"}, InterBlockRouteEndpoint{Ref: "R1", Pin: "1"})
	tree := BuildInterBlockRouteTree(group, routeTreeTestTargets("SIG", map[string]transactions.Point{"U1.1": {XMM: 0, YMM: 0}, "J1.1": {XMM: 40, YMM: 0}, "R1.1": {XMM: 5, YMM: 0}}))
	if tree.RootEndpointID != "J1.1" || len(tree.Branches) != 2 {
		t.Fatalf("tree = %#v, want connector root with two branches", tree)
	}
}

func TestBuildInterBlockRouteTreeUsesCentralEndpointFallback(t *testing.T) {
	group := routeTreeTestGroup("SIG", InterBlockRouteEndpoint{Ref: "U1", Pin: "1"}, InterBlockRouteEndpoint{Ref: "R1", Pin: "1"}, InterBlockRouteEndpoint{Ref: "C1", Pin: "1"})
	tree := BuildInterBlockRouteTree(group, routeTreeTestTargets("SIG", map[string]transactions.Point{"U1.1": {XMM: 0, YMM: 0}, "R1.1": {XMM: 5, YMM: 0}, "C1.1": {XMM: 10, YMM: 0}}))
...
func TestBuildInterBlockRouteTreePlansNearestBranchesDeterministically(t *testing.T) {
	group := routeTreeTestGroup("SIG", InterBlockRouteEndpoint{Ref: "J1", Pin: "1"}, InterBlockRouteEndpoint{Ref: "U1", Pin: "1"}, InterBlockRouteEndpoint{Ref: "R1", Pin: "1"}, InterBlockRouteEndpoint{Ref: "C1", Pin: "1"})
	tree := BuildInterBlockRouteTree(group, routeTreeTestTargets("SIG", map[string]transactions.Point{"J1.1": {XMM: 0, YMM: 0}, "U1.1": {XMM: 10, YMM: 0}, "R1.1": {XMM: 3, YMM: 0}, "C1.1": {XMM: 4, YMM: 0}}))
	got := []string{}
...
func TestBuildInterBlockRouteTreeReportsMissingTargets(t *testing.T) {
	group := routeTreeTestGroup("SIG", InterBlockRouteEndpoint{Ref: "J1", Pin: "1"}, InterBlockRouteEndpoint{Ref: "U1", Pin: "1"})
	tree := BuildInterBlockRouteTree(group, routeTreeTestTargets("SIG", map[string]transactions.Point{"J1.1": {XMM: 0, YMM: 0}}))
	if tree.RootEndpointID != "J1.1" || len(tree.Branches) != 0 || len(tree.MissingEndpointIDs) != 1 || tree.MissingEndpointIDs[0] != "U1.1" {
...
func TestBuildInterBlockRouteTreesPreservesCaseSensitiveNetNames(t *testing.T) {
	group := routeTreeTestGroup("vcc", InterBlockRouteEndpoint{Ref: "J1", Pin: "1"}, InterBlockRouteEndpoint{Ref: "U1", Pin: "1"})
	trees := BuildInterBlockRouteTrees([]InterBlockRouteGroup{group}, InterBlockContactEvidence{Targets: []InterBlockContactTarget{{NetName: "VCC", Kind: InterBlockContactTargetPad, Ref: "J1", Pad: "1", Point: transactions.Point{XMM: 0}, Layer: "F.Cu", Confidence: InterBlockContactConfidenceHigh}, {NetName: "VCC", Kind: InterBlockContactTargetPad, Ref: "U1", Pad: "1", Point: transactions.Point{XMM: 10}, Layer: "F.Cu", Confidence: InterBlockContactConfidenceHigh}}})
	if len(trees) != 1 || trees[0].TargetCount != 0 || len(trees[0].MissingEndpointIDs) != 2 {
		t.Fatalf("trees = %#v, want no case-insensitive match", trees)
	}
}

func TestBuildInterBlockRouteTreesSelectsDuplicateTargetsDeterministically(t *testing.T) {
	group := routeTreeTestGroup("SIG", InterBlockRouteEndpoint{Ref: "J1", Pin: "1"}, InterBlockRouteEndpoint{Ref: "U1", Pin: "1"})
	targets := []InterBlockContactTarget{{NetName: "SIG", Kind: InterBlockContactTargetPad, Ref: "J1", Pad: "1", Point: transactions.Point{XMM: 20}, Layer: "F.Cu", Confidence: InterBlockContactConfidenceMedium}, {NetName: "SIG", Kind: InterBlockContactTargetPad, Ref: "J1", Pad: "1", Point: transactions.Point{}, Layer: "F.Cu", Confidence: InterBlockContactConfidenceHigh}, {NetName: "SIG", Kind: InterBlockContactTargetPad, Ref: "U1", Pad: "1", Point: transactions.Point{XMM: 10}, Layer: "F.Cu", Confidence: InterBlockContactConfidenceHigh}}
	...
	if len(tree.Branches) != 1 || tree.Branches[0].PlannedDistanceMM != 10 {
		t.Fatalf("tree = %#v, want high-confidence target", tree)
	}
}
...
func TestBuildInterBlockRouteTreesI2CSensorBreakoutPrunesLocallyRoutedPassives(t *testing.T) {
	...
	for _, tree := range trees {
		if tree.RootEndpointID == "" || tree.RequiredEndpointCount != 2 || tree.TargetCount != tree.RequiredEndpointCount || len(tree.Branches) != tree.RequiredEndpointCount-1 {
			t.Fatalf("tree = %#v, want connector-to-sensor coverage", tree)
		}
	}
}

func routeTreeTestGroup(netName string, endpoints ...InterBlockRouteEndpoint) InterBlockRouteGroup {
	groups, issues := BuildInterBlockRouteGroups([]InterBlockRouteCandidate{{NetName: netName, Status: InterBlockRouteCandidateRoutable, Endpoints: endpoints}})
	if len(issues) != 0 || len(groups) != 1 {
...
func routeTreeTestTargets(netName string, points map[string]transactions.Point) map[string]InterBlockContactTarget {
	...
	for id, point := range points {
		...
		targets[normalizedRouteGroupEndpointKey(ref, pin)] = InterBlockContactTarget{NetName: netName, Kind: InterBlockContactTargetPad, Ref: ref, Pad: pin, Point: point, Layer: "F.Cu", Confidence: InterBlockContactConfidenceHigh}
	}
	return targets
}
