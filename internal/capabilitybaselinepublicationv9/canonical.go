...
func withSingleTrailingLF(data []byte) []byte {
...
func rejectJSONMaps(typeOf reflect.Type, seen map[reflect.Type]bool) error {
	...
	if typeOf.Kind() == reflect.Map {
		return fmt.Errorf("map-backed JSON is prohibited from V9 baseline artifacts")
	}
	if typeOf.Kind() == reflect.Interface {
		return fmt.Errorf("interface-backed JSON is prohibited from V9 baseline artifacts")
	}
	switch typeOf.Kind() {
	case reflect.Struct:
