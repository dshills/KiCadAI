...
func checkSemanticBoundaryProviderRequest(t testing.TB, r *http.Request, prompt string) int {
	t.Helper()
	if r.Method != "POST" || r.URL.String() != "https://api.openai.com/v1/responses" || r.ContentLength <= 0 || r.ContentLength > SemanticBoundaryMaxRequestBytes {
		t.Fatal("semantic-boundary endpoint or size bound differs")
	}
	...
	want, source, err := prepareSemanticBoundaryGenerateRequest(prompt)
	if err != nil {
		t.Fatal(err)
	}
	if payload.Model != GroundedFullModel || payload.MaxOutputTokens != 1600 || payload.Store || payload.Background || !payload.Stream ||
		!payload.Text.Format.Strict || payload.Text.Format.Type != "json_schema" || payload.Text.Format.Name != SemanticBoundarySchemaName ||
		...
		t.Fatal("semantic-boundary wire contract differs")
	}
	...
	contract, err := SemanticBoundaryEvidenceContract(prompt)
...
func TestSemanticBoundaryProviderCorpusWireAndJournal(t *testing.T) {
	...
	minimum, maximum := SemanticBoundaryMaxRequestBytes, 0
	for _, c := range corpus.Cases {
		t.Run(c.ID, func(t *testing.T) {
			root := t.TempDir()
			journal := filepath.Join(root, "journal")
			calls := 0
			s, err := InterpretSemanticBoundaryWithJournal(context.Background(), c.Prompt, filepath.Join(root, "ledger.json"), LedgerPolicy{"offline-boundary-wire", 1, 50000}, roundTripperFunc(func(r *http.Request) (*http.Response, error) {
				calls++
				n := checkSemanticBoundaryProviderRequest(t, r, c.Prompt)
				minimum, maximum = min(minimum, n), max(maximum, n)
				return referencedProviderResponseForModel(t, []byte(`{}`), "refusal", "offline-boundary-"+c.ID, GroundedFullModel), nil
			}), journal)
			if err == nil || calls != 1 || s.Outcome != "provider_refusal" || s.Model != GroundedFullModel || s.AdmissionVersion != SemanticBoundaryVersion {
				t.Fatal("incorrect synthetic provider refusal", err, calls, s.Outcome)
			}
			a, err := InspectSemanticBoundaryJournal(journal)
			if err != nil || a.Version != "semantic-boundary-journal-audit-1" || a.Ledger.Version != 3 || a.Ledger.AccountingProfile != SemanticBoundaryAccountingProfile || a.Ledger.Entries[0].EstimatedMicroUSD != 1800 {
				t.Fatal("v10 accounting/audit differs", err)
			}
			for _, p := range []extractionProtocol{indexedProtocol, ownedProtocol, connectionProtocol, directProtocol, groundedProtocol, groundedFullProtocol, sourceEligibleProtocol, sourceAddressedProtocol} {
				if _, err := inspectProtocolJournal(journal, p); err == nil {
					t.Fatal("historical inspector accepted v10 journal")
				}
			}
		})
	}
...
func testBoundedProviderPreflight(t *testing.T, protocol extractionProtocol, check func(testing.TB, *http.Request, string) int) {
	...
	for _, mode := range []string{"no-budget", "no-key", "nil-transport", "no-ledger", "no-journal", "existing-journal", "oversize", "wrong-model", "transport-error", "incomplete"} {
		t.Run(mode, func(t *testing.T) {
			...
			policy := LedgerPolicy{"offline-boundary-gates", 2, 100000}
			calls, want := 0, 0
			var base http.RoundTripper = roundTripperFunc(func(r *http.Request) (*http.Response, error) {
				...
				return referencedProviderResponseForModel(t, []byte(`{}`), mode, "offline-boundary-gate", GroundedFullModel), nil
			})
			switch mode {
			case "no-budget":
				policy = LedgerPolicy{}
...
func TestSemanticBoundaryJournalTampering(t *testing.T) {
	testRequestEvidenceJournalTampering(t, semanticBoundaryProtocol)
}

func TestSemanticBoundaryHistoricalV9JournalIsolation(t *testing.T) {
	baseline := os.Getenv("KICADAI_BOUNDARY_V9_BASELINE")
	if baseline == "" {
		t.Skip("optional read-only historical v9 evidence path not supplied")
	}
	...
	for _, c := range corpus.Cases {
		t.Run(c.ID, func(t *testing.T) {
			...
			a, err := InspectSourceAddressedJournal(root)
			if err != nil || a.Selection.OriginalRequest != c.Prompt || a.Selection.AdmissionVersion != SourceAddressedVersion || a.Ledger.AccountingProfile != SourceAddressedAccountingProfile {
				t.Fatal("historical v9 evidence no longer replays", err)
			}
			if _, err := InspectSemanticBoundaryJournal(root); err == nil {
				t.Fatal("historical v9 evidence accepted as new v10 output")
			}
		})
	}
}
