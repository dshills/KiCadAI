...
// synthesisValueTrialV14 deliberately retains only the compact catalog trial.
// The candidate graph is materialized on demand for the current evaluation.
type synthesisValueTrialV14 struct {
	candidateIndex int
	trialIndex     int
	trial          ValueTrial
}

// SynthesizeV14 preserves the frozen bounded synthesis behavior while
// materializing at most one queued value-trial graph at a time.
func SynthesizeV14(
	ctx context.Context,
	...
	valueTrials := make([][]synthesisValueTrialV14, len(run.Search.Candidates))
	...
	for candidateIndex, candidate := range run.Search.Candidates {
		...
		for trialIndex := range enumeration.Trials {
			...
			valueTrials[candidateIndex] = append(valueTrials[candidateIndex], synthesisValueTrialV14{
