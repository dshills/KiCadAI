...
// synthesisValueTrialV17 deliberately retains only the compact catalog trial.
// The candidate graph is materialized on demand for the current evaluation.
type synthesisValueTrialV17 struct {
	candidateIndex int
	trialIndex     int
	trial          ValueTrial
}

// SynthesizeV17 preserves the frozen bounded synthesis behavior while
...
func SynthesizeV17(
	...
	valueTrials := make([][]synthesisValueTrialV17, len(run.Search.Candidates))
	...
	for candidateIndex, candidate := range run.Search.Candidates {
		...
		for trialIndex := range enumeration.Trials {
			...
			valueTrials[candidateIndex] = append(valueTrials[candidateIndex], synthesisValueTrialV17{
	...
	for trialIndex := 0; trialIndex < maximumTrialCount; trialIndex++ {
		for _, candidateIndex := range candidateOrder {
			...
			evaluation := EvaluateCandidateV17(
			...
			case SimulationEvaluationFailed:
				...
				if !hasFailure[candidateIndex] || compareSynthesisFailureV17(failure, bestFailures[candidateIndex]) < 0 {
	...
	if len(passes) != 0 {
		return selectRankedSynthesisResultV17(run, passes)
	}
	...
	slices.SortFunc(failures, compareSynthesisFailureV17)
...
func compareSynthesisFailureV17(left, right synthesisFailure) int {
	return cmp.Or(
...
func selectRankedSynthesisResultV17(run SynthesisRun, passes []synthesisPassingCandidate) SynthesisRun {
...
func finalizeSynthesisRunV17(run SynthesisRun) SynthesisRun {
