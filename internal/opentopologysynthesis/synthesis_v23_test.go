...
func TestElectricalSynthesisV23ContinuesCompleteCertificateAndLowers(t *testing.T) {
	...
	result := ContinueElectricalV23(context.Background(), r, v21, inventory, environment, admission, DefaultPolicy(), DefaultElectricalRepairLimitsV22())
	...
	if result.Synthesis.Report.Selected.EvaluationHash != result.ElectricalRepair.Selected.Evaluation.Evaluation.Hash {
	...
	replay := ContinueElectricalV23(context.Background(), r, v21, inventory, environment, admission, DefaultPolicy(), DefaultElectricalRepairLimitsV22())
	if !reflect.DeepEqual(result, replay) {
		t.Fatal("synthesis continuation is not deterministic")
	}
}
func TestElectricalSynthesisV23PreservesIneligibleAndTamperedSources(t *testing.T) {
	r, graph, inventory, environment, admission := testRepairFixtureV23(t)
	...
	for _, category := range []string{"passed", "incomplete", "canceled", "invalid", "certificate", "evaluation", "requirement", "source_hash", "source_metadata"} {
		t.Run(category, func(t *testing.T) {
			...
			result := ContinueElectricalV23(context.Background(), req, original, inventory, environment, admission, DefaultPolicy(), DefaultElectricalRepairLimitsV22())
...
func TestElectricalSynthesisV23IndependentOptionalKiCadPromotion(t *testing.T) {
	...
	for _, compound := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			v21 := testCompletedV21RunV22(t, r, graph, inventory, environment, admission)
			result := ContinueElectricalV23(ctx, r, v21, inventory, environment, admission, DefaultPolicy(), DefaultElectricalRepairLimitsV22())
			...
			if retained := strings.TrimSpace(os.Getenv("KICADAI_OPEN_TOPOLOGY_ARTIFACT_ROOT")); retained != "" {
				root = filepath.Join(retained, "v23_"+name)
			}
