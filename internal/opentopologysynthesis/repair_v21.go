...
func repairCandidatePreparedV21(
	...
	unsignedInitial := initial
	unsignedInitial.Hash = ""
	initialData, initialErr := json.Marshal(unsignedInitial)
	initialDigest := sha256.Sum256(initialData)
	if initial.GraphHash != result.InitialGraphHash || initial.RequirementHash != result.RequirementHash ||
		initial.InventoryHash != inventory.Hash || initial.Hash == "" || initialErr != nil || initial.Hash != hex.EncodeToString(initialDigest[:]) {
		result.Status = RepairSearchUnsupported
