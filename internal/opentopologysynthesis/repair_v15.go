...
func RepairCandidateV15(
	...
	if err != nil {
		...
		return finalizeRepairSearchV15(result)
	}
	...
	for len(frontier) != 0 {
		...
		analysis, causalCandidates := analyzeCausalRepairsV15(
...
func finalizeRepairSearchV15(result RepairSearchResult) RepairSearchResult {
	result.Trace = electricalRepairTrace(result)
	copy := result
	copy.Hash = ""
	result.Hash = hashJSONV15(copy)
	return result
}

func hashJSONV15(value any) string {
	hasher := sha256.New()
	if err := canonicaljsonstream.Encode(hasher, value); err != nil {
		panic("canonical V15 repair hash encoding failed: " + err.Error())
	}
	return hex.EncodeToString(hasher.Sum(nil))
}

func analyzeCausalRepairsV15(
	ctx context.Context,
	requirement Requirement,
	graph CandidateGraph,
	initial SimulationEvaluation,
	...
	candidates := generateCausalCandidatesV15(requirement, graph, initial, inventory, policy)
...
func generateCausalCandidatesV15(
	requirement Requirement,
	graph CandidateGraph,
	initial SimulationEvaluation,
	inventory PrimitiveInventory,
	...
	for _, proposal := range generateRepairProposals(requirement, graph, initial.Diagnoses, inventory, policy) {
		...
		result = append(result, sizedCausalProposalCandidatesV15(
...
func sizedCausalProposalCandidatesV15(
	requirement Requirement,
	...
	visitCausalValueCandidatesV15(requirement, proposed, inventory, policy, func(valueCandidate causalCandidate) bool {
...
func visitCausalValueCandidatesV15(
	requirement Requirement,
	graph CandidateGraph,
	inventory PrimitiveInventory,
	policy Policy,
