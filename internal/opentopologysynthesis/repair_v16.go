...
func RepairCandidateV16(
	...
	if err != nil {
		...
		return finalizeRepairSearchV16(result)
	}
	...
	for len(frontier) != 0 {
		...
		analysis, causalCandidates := analyzeCausalRepairsV16(
...
func finalizeRepairSearchV16(result RepairSearchResult) RepairSearchResult {
	result.Trace = electricalRepairTrace(result)
	copy := result
	copy.Hash = ""
	result.Hash = hashJSONV16(copy)
	return result
}

func hashJSONV16(value any) string {
	hasher := sha256.New()
	if err := canonicaljsonstream.Encode(hasher, value); err != nil {
		panic("canonical V16 repair hash encoding failed: " + err.Error())
	}
	return hex.EncodeToString(hasher.Sum(nil))
}

func analyzeCausalRepairsV16(
	ctx context.Context,
	requirement Requirement,
	graph CandidateGraph,
	initial SimulationEvaluation,
	...
	candidates := generateCausalCandidatesV16(requirement, graph, initial, inventory, policy)
...
func generateCausalCandidatesV16(
	requirement Requirement,
	graph CandidateGraph,
	initial SimulationEvaluation,
	inventory PrimitiveInventory,
	...
	for _, proposal := range generateRepairProposals(requirement, graph, initial.Diagnoses, inventory, policy) {
		...
		result = append(result, sizedCausalProposalCandidatesV16(
...
func sizedCausalProposalCandidatesV16(
	requirement Requirement,
	...
	visitCausalValueCandidatesV16(requirement, proposed, inventory, policy, func(valueCandidate causalCandidate) bool {
...
func visitCausalValueCandidatesV16(
	requirement Requirement,
	graph CandidateGraph,
	inventory PrimitiveInventory,
	policy Policy,
