...
func auditBytesV9(manifest ManifestV9, manifestHash string, discovery DiscoveryObligationsV9, heldOut HeldOutObligationCommitmentV9) []byte {
	var output strings.Builder
	output.WriteString("# Closed-Loop Open-Set V9 Corpus Freeze Audit\n\n")
	...
	output.WriteString("\nThe 32-byte V9 held-out source key was created exclusively outside the repository. Held-out records use independently authenticated unique random nonces. Discovery anchors are public; held-out anchors are represented only by their aggregate commitment. All artifacts are bound by the publication checksum manifest.\n")
