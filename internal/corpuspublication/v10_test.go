...
func TestPublishV10RecordSealsAndObligations(t *testing.T) {
	request, sources := publicationFixtureV10(t)
	result, err := PublishV10(request)
	if err != nil {
		t.Fatal(err)
	}
	if result.Manifest.Schema != ManifestSchemaV10 || result.Manifest.Version != 10 || result.DiscoveryCases != 24 || result.HeldOutCases != 24 ||
		...
		t.Fatalf("unexpected V10 result: %+v", result)
	}
	if err := verifyChecksums(request.DestinationRoot); err != nil {
		t.Fatal(err)
	}
	publicVerification, err := VerifyPublicationV10(request.DestinationRoot)
	if err != nil || publicVerification.DiscoveryCases != 24 || publicVerification.HeldOutCases != 24 {
		t.Fatalf("verify public V10 publication: %+v, %v", publicVerification, err)
	}
	manifestBytes, err := os.ReadFile(filepath.Join(request.DestinationRoot, ManifestFileV10))
	if err != nil {
		t.Fatal(err)
	}
	if hashBytes(manifestBytes) != result.ManifestSHA256 {
		t.Fatal("V10 manifest hash mismatch")
	}
	validationBytes, err := os.ReadFile(filepath.Join(request.DestinationRoot, ValidationFileV10))
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, name := range []string{ManifestFileV10, ValidationFileV10, DiscoveryObligationsFileV10, AuthorshipAttestationsFileV10, AuditFileV10} {
		data, err := os.ReadFile(filepath.Join(request.DestinationRoot, name))
		if err != nil {
			t.Fatal(err)
		}
		if bytes.Contains(data, []byte("v10_case_025")) || bytes.Contains(data, []byte("held_out/request_025")) {
	...
	authorshipBytes, err := os.ReadFile(filepath.Join(request.DestinationRoot, AuthorshipAttestationsFileV10))
	...
	if err := json.Unmarshal(authorshipBytes, &authorship); err != nil || authorship.Version != 10 || len(authorship.Records) != expectedAuthorsV10 {
		t.Fatalf("sanitized authorship attestations malformed: %v", err)
	}
	for _, forbidden := range []string{"author_context_identity", "authoring_tool_model_version", "returned_bundle_root", "requirement_source_sha256", "held_out/request_025", "v10_case_025"} {
	...
	if err != nil || info.Mode().Perm() != 0o600 || len(key) != 32 {
		t.Fatalf("V10 key metadata invalid")
	}
	ciphertext, err := os.ReadFile(filepath.Join(request.DestinationRoot, HeldOutCipherFileV10))
	if err != nil {
		t.Fatal(err)
	}
	opened, err := OpenHeldOutV10(key, result.Manifest, ciphertext)
	if err != nil || len(opened) != 24 {
		t.Fatalf("open V10 records: %d, %v", len(opened), err)
	}
	...
	keyedVerification, err := VerifyPublicationV10WithKey(request.DestinationRoot, key)
	if err != nil || keyedVerification.ManifestSHA256 != result.ManifestSHA256 || keyedVerification.HeldOutCases != 24 {
		t.Fatalf("verify keyed V10 publication: %+v, %v", keyedVerification, err)
	}
	if _, err := VerifyPublicationV10WithKey(request.DestinationRoot, bytes.Repeat([]byte{0xff}, 32)); err == nil {
		t.Fatal("wrong V10 source key was accepted")
	}

	var discovery DiscoveryObligationsV10
	data, err := os.ReadFile(filepath.Join(request.DestinationRoot, DiscoveryObligationsFileV10))
	if err != nil {
		t.Fatal(err)
	}
	...
	var heldOut HeldOutObligationCommitmentV10
	data, err = os.ReadFile(filepath.Join(request.DestinationRoot, HeldOutCommitmentFileV10))
	if err != nil {
		t.Fatal(err)
	}
...
func TestVerifyPublicationV10RejectsUnexpectedFileAndCrossVersionRecord(t *testing.T) {
	request, _ := publicationFixtureV10(t)
	_, err := PublishV10(request)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := VerifyPublicationV10(request.DestinationRoot); err == nil {
		t.Fatal("V10 verifier accepted an unexpected artifact")
	}
	item := heldOutCaseV10{Entry: EntryV10{ID: "v10_case_025", Role: "held_out", StablePath: "held_out/v10_case_025.json", Sealed: true,
		RequirementSHA256: hashBytes([]byte("source"))}, Source: []byte("source")}
	record, err := encodeHeldOutRecordV10(item)
	if err != nil {
		t.Fatal(err)
	}
	if _, err := decodeHeldOutRecordV8(record); err == nil {
		t.Fatal("V8 decoder accepted a V10 held-out record")
	}
}

func TestOpenHeldOutV10RejectsRecordAndAADTampering(t *testing.T) {
	request, _ := publicationFixtureV10(t)
	result, err := PublishV10(request)
	if err != nil {
		t.Fatal(err)
	...
	ciphertext, _ := os.ReadFile(filepath.Join(request.DestinationRoot, HeldOutCipherFileV10))
	tampered := append([]byte(nil), ciphertext...)
	tampered[len(tampered)-1] ^= 1
	if _, err := OpenHeldOutV10(key, result.Manifest, tampered); err == nil {
		t.Fatal("tampered V10 record set accepted")
	}
	metadata := result.Manifest
	metadata.PacketSetSHA256 = strings.Repeat("a", 64)
	if _, err := OpenHeldOutV10(key, metadata, ciphertext); err == nil {
		t.Fatal("tampered V10 AAD metadata accepted")
	}
	metadata = result.Manifest
	metadata.AuthorshipAttestationsSHA256 = strings.Repeat("b", 64)
	if _, err := OpenHeldOutV10(key, metadata, ciphertext); err == nil {
		t.Fatal("tampered V10 authorship AAD metadata accepted")
	}
}

func TestPublishV10FailsClosedOnOccupiedDestinationAndEntropy(t *testing.T) {
	request, _ := publicationFixtureV10(t)
	if err := os.MkdirAll(request.DestinationRoot, 0o755); err != nil {
		t.Fatal(err)
	}
	if _, err := PublishV10(request); err == nil {
		t.Fatal("occupied V10 destination accepted")
	}
	if _, err := os.Lstat(request.KeyPath); !os.IsNotExist(err) {
		t.Fatal("refused V10 publication created a key")
	}

	request, _ = publicationFixtureV10(t)
	request.Random = bytes.NewReader(make([]byte, 40))
	if _, err := PublishV10(request); err == nil {
		t.Fatal("short V10 entropy accepted")
	}
	if _, err := os.Lstat(request.DestinationRoot); !os.IsNotExist(err) {
		t.Fatal("failed V10 publication left destination")
	}
	if _, err := os.Lstat(request.KeyPath); !os.IsNotExist(err) {
		t.Fatal("failed V10 publication left key")
	}
}

func publicationFixtureV10(t *testing.T) (RequestV10, map[string][]byte) {
	t.Helper()
	root := t.TempDir()
	...
	report := corpusfreezev10.Report{Schema: "kicadai.behavior-corpus-validation-report.v10", Version: 10,
		PolicySHA256: hashBytes([]byte("policy-v10")), PacketSetSHA256: hashBytes([]byte("packet-v10")),
		ContractBindingSHA256: hashBytes([]byte("binding-v10")), HistoricalCommitmentsSHA256: hashBytes([]byte("history-v10")),
	...
	for authorIndex := 1; authorIndex <= expectedAuthorsV10; authorIndex++ {
	...
	for index := 1; index <= expectedCasesV10; index++ {
		...
		sources[id] = source
		report.Entries = append(report.Entries, corpusfreezev10.EntryEvidence{ID: id, AuthorSlot: author, Role: role, Domain: "analog_signal_path", CircuitRole: "conversion_regulation", SafetyImpact: "non_safety",
			PrimaryClass: "static", RequiredPrimaryAnalysis: "dc_operating_point", OutputMultiplicity: "single", RequireOffNominal: false,
			SourceID: fmt.Sprintf("v10_source_%03d", index), RequirementFile: requestPath, RequirementSHA256: hashBytes(source),
	...
	for authorIndex := 1; authorIndex <= expectedAuthorsV10; authorIndex++ {
		...
		sourceHashes := make([]corpusfreezev10.SourceHash, 0, len(paths))
		for _, path := range paths {
			sourceHashes = append(sourceHashes, corpusfreezev10.SourceHash{Path: path, SHA256: hashBytes(bundle.Requirements[path])})
		}
		authorship := corpusfreezev10.Authorship{Schema: "kicadai.closed-loop-open-set-authorship.v10", Version: 10,
			...
			RequirementSourceSHA256: sourceHashes, Uncertainties: []string{}, Attestations: allTrueAuthorshipAttestationsV10Fixture()}
	...
	return RequestV10{RepositoryRoot: repository, DestinationRoot: filepath.Join(repository, "corpus"), KeyPath: filepath.Join(root, "keys", "v10-source.key"),
		ContractManifestSHA256: hashBytes([]byte("contract-v10")), ValidatorManifest: []byte("validator-v10\n"), PublisherManifest: []byte("publisher-v10\n"),
...
func allTrueAuthorshipAttestationsV10Fixture() corpusfreezev10.AuthorshipAttestations {
	return corpusfreezev10.AuthorshipAttestations{
