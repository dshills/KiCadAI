...
type Operation struct {
	...
	// Rebuildable is in-memory placement/routing metadata. It marks authored
	// local copper whose endpoints moved independently and therefore must be
	// regenerated by the obstacle-aware router before serialization.
	Rebuildable         bool     `json:"-"`
	RebuildRefs         []string `json:"-"`
	RebuildSourceLayers []string `json:"-"`
	RebuildTargetLayers []string `json:"-"`
	// PruneProtected is in-memory routing-stage metadata. It follows an
	// operation through order-preserving rewrites so dangling-stub cleanup does
	// not rely on stale slice indices from an earlier representation.
	PruneProtected bool `json:"-"`
}
...
func (op Operation) Clone() Operation {
	...
	clone.RebuildRefs = append([]string(nil), op.RebuildRefs...)
	clone.RebuildSourceLayers = append([]string(nil), op.RebuildSourceLayers...)
	clone.RebuildTargetLayers = append([]string(nil), op.RebuildTargetLayers...)
	return clone
}
