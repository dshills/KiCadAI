...
func TestNativeJointFrozenCandidateDiagnostic(t *testing.T) {
	root := os.Getenv("KICADAI_JOINT_DIAGNOSTIC_INPUT")
	if root == "" {
	...
	derived := os.Getenv("KICADAI_JOINT_DIAGNOSTIC_TRANSACTION")
	if derived == "" {
	...
	if derived == "" && (placementErr == nil || !strings.Contains(placementErr.Error(), "bounded candidate visits")) {
		t.Fatalf("expected preserved bounded candidate conflict, got %v", placementErr)
	}
