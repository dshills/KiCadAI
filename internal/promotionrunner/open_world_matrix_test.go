...
func TestOpenWorldCapabilityPromotionMatrixUsesRequirementLane(t *testing.T) {
	root, err := filepath.Abs(filepath.Join("..", ".."))
	if err != nil {
		t.Fatal(err)
	}
	document, err := LoadMatrix(
		filepath.Join(root, "specs", "open-world-capability-evaluation", "PROMOTION_MATRIX.json"),
		root,
	)
	if err != nil {
		t.Fatal(err)
