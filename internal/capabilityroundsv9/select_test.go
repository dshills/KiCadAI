...
func TestSelectCountsClosureAndRanksCaseDomainRoleBreadth(t *testing.T) {
	...
	if selection.CandidateCount != 3 || len(selection.Selected.FullyCoveredCaseIDs) != 4 || len(selection.Selected.ReportingDomains) != 4 || len(selection.Selected.CircuitRoles) != 4 || len(selection.Selected.Atoms) != 3 || len(selection.Selected.Members) != 3 {
...
func TestCompleteTwentyFourCaseClosureFitsFrozenCeilingAndPolicyCannotRelax(t *testing.T) {
	...
	cases := make([]Case, 0, 24)
	active := make([]string, 0, 24)
