...
type exposureProjection struct {
	fullyCovered      []string
	exposed           []string
	exposures         []CaseExposure
	nonExposed        []CaseCommitment
	domains           []string
	roles             []string
	safetyWeight      int
	exposedNoncovered int
	siblingCount      int
}
...
func Select(cases []Case, plans []EffectPlan, state RoundState, policy Policy) (Selection, error) {
	...
	// V9 freezes exactly 24 discovery cases and complete enumeration. Keep the
...
func validateGap(gap Gap, policy Policy) (string, error) {
	...
	if _, err := sortedUnique(gap.Diagnostics); err != nil || len(gap.Diagnostics) == 0 {
		return "", fmt.Errorf("%w: gap diagnostics", ErrInvalidInput)
	}
...
func buildCandidate(seed directSeed, plan EffectPlan, cases []normalizedCase, atomSupport map[string]int, priorAtoms map[string]bool, state RoundState, policy Policy) (Candidate, error) {
	...
	directMemberAtoms := map[string]bool{}
	for _, member := range seed.members {
		atomKey, _ := AtomKey(member.Category, member.Scope, member.Capability)
		if seed.atoms[atomKey].Key == "" {
			return Candidate{}, fmt.Errorf("%w: direct member has no direct atom", ErrInvalidInput)
		}
		directMemberAtoms[atomKey] = true
	}
	if !subset(directAtomKeys, directMemberAtoms) {
		return Candidate{}, fmt.Errorf("%w: direct atom has no direct member", ErrInvalidInput)
	}
	...
	for _, atom := range plan.ClosureAtoms {
		...
		if err != nil || atom.Key != key || atoms[key].Key != "" || seenClosureAtoms[key] || priorClosureAtomKey >= key && priorClosureAtomKey != "" {
	...
	for _, member := range plan.ClosureMembers {
		...
		if err != nil || atomErr != nil || member.Key != key || atoms[atomKey].Key == "" || members[key].Key != "" || seenClosureMembers[key] || priorClosureMemberKey >= key && priorClosureMemberKey != "" {
	...
	evidence := map[string]bool{}
	coveredSet := stringSet(projection.fullyCovered)
	for _, current := range cases {
		if coveredSet[current.value.ID] {
	...
	if len(projection.fullyCovered) < policy.MinimumCaseSupport || len(projection.domains) < policy.MinimumDomains || len(projection.roles) < policy.MinimumRoles {
		return Candidate{}, nil
	}
	...
	candidate := Candidate{DirectAtomKeys: directAtomKeys, DirectMemberKeys: directMemberKeys, Atoms: atomValues(atoms), Members: memberValues(members),
		FullyCoveredCaseIDs: projection.fullyCovered, EffectExposureCaseIDs: projection.exposed, Exposure: projection.exposures, NonExposedCases: projection.nonExposed,
		ReportingDomains: projection.domains, CircuitRoles: projection.roles, SafetyWeight: projection.safetyWeight,
		ExposedNoncoveredCaseCount: projection.exposedNoncovered, NonselectedSiblingPathCount: projection.siblingCount,
		RequiredEvidence: sortedSet(evidence), EffectPlanSHA256: plan.PlanSHA256}
...
func compareSemantics(left, right Candidate) int {
	for _, order := range []int{
		cmp.Compare(len(right.FullyCoveredCaseIDs), len(left.FullyCoveredCaseIDs)),
		...
		cmp.Compare(left.ExposedNoncoveredCaseCount, right.ExposedNoncoveredCaseCount),
		cmp.Compare(left.NonselectedSiblingPathCount, right.NonselectedSiblingPathCount),
...
func deriveExposureProjection(cases []normalizedCase, atomKeys, memberKeys []string, policy Policy) (exposureProjection, error) {
	effectiveAtoms, effectiveMembers := stringSet(atomKeys), stringSet(memberKeys)
	covered, exposed := []string{}, []string{}
	exposures := []CaseExposure{}
	nonExposed := []CaseCommitment{}
	domains, roles := map[string]bool{}, map[string]bool{}
	safety := 0
	for _, current := range cases {
		selectedPaths, siblingPaths := []string{}, []string{}
		for _, gap := range current.value.Frontier {
			pathHash, err := PathHash(gap)
			if err != nil {
			...
			atomKey, _ := AtomKey(leaf.Category, leaf.Scope, leaf.Capability)
			memberKey, _ := MemberKey(leaf)
			if effectiveAtoms[atomKey] || effectiveMembers[memberKey] {
				selectedPaths = append(selectedPaths, pathHash)
			} else {
				siblingPaths = append(siblingPaths, pathHash)
			}
		}
		sort.Strings(selectedPaths)
		sort.Strings(siblingPaths)
		if len(selectedPaths) != 0 {
			exposed = append(exposed, current.value.ID)
			exposures = append(exposures, CaseExposure{CaseID: current.value.ID, SelectedPathHashes: selectedPaths, NonselectedSiblingPathHashes: siblingPaths})
		} else {
			caseHash, err := CaseHash(current.value)
			if err != nil {
				return exposureProjection{}, err
			}
			nonExposed = append(nonExposed, CaseCommitment{CaseID: current.value.ID, CaseSHA256: caseHash})
		}
		if policy.EligibleOutcomes[current.value.Outcome] && subset(current.memberKeys, effectiveMembers) {
	...
	coveredSet := stringSet(covered)
	exposedNoncovered, siblingCount := 0, 0
	for _, exposure := range exposures {
		if !coveredSet[exposure.CaseID] {
			exposedNoncovered++
		}
		siblingCount += len(exposure.NonselectedSiblingPathHashes)
	}
	return exposureProjection{fullyCovered: covered, exposed: exposed, exposures: exposures, nonExposed: nonExposed,
		domains: sortedSet(domains), roles: sortedSet(roles), safetyWeight: safety,
		exposedNoncovered: exposedNoncovered, siblingCount: siblingCount}, nil
}
