...
func BuildProviderContext(prompt string, capabilities json.RawMessage) (string, error) {
	...
	context := ProviderContext{
		...
		Policy: []string{
			...
			"the target is requirement v3: control fields are null, control_transitions and events are empty, transition references are empty; later-version control/event semantics require a capability gap, never guessed substitutes",
			"coverage disposition is compiled, clarification, capability_gap, or context; reference kind is requirement, uncertainty, clarification, or capability_gap; never use objective/domain/port/signal as reference kinds",
			"compiled coverage references requirement IDs and optional uncertainty IDs; clarification coverage references clarification and uncertainty IDs; capability_gap coverage references capability_gap and uncertainty IDs; context has no references and must not discard a material source condition",
			"reference every material domain, external port, participant, objective, operating case, and behavioral requirement from the original source; every uncertainty and blocker also needs source coverage",
			"use normalized lowercase semantic IDs, not prose, in identity/capability/kind fields; describe the limitation in capability_gaps.reason and required_evidence",
			"a clarification owns every unresolved uncertainty at one exact path; each uses resolution clarification and resolved_by equal to that clarification ID; list all owned uncertainty IDs in the question",
			...
			"domain source is external, a declared supply signal ID, or port:<id> for a generated external power output in that domain; use the registered metric/analysis/unit and operating-axis/unit combinations exactly; cross-reference existence and numeric bound ordering are compiler-enforced",
			...
			"voltage_regulation must produce its derived supply through a declared power signal or a real external power output with source port:<output_id>; the output port must be source-direction power in the same domain with one objective bound through role output; other objectives may consume it through input, power or sense roles; an independent external supply is not a regulated output; preserve coherent source/sink bindings and do not add a fictitious consumer or function to make a signal valid",
			"v3 observations are port, signal, domain, circuit, or participant_port; circuit ID is exactly circuit; participant_port ID is exactly participant_id.port_id for a declared scalar port; bundled digital_bus or differential_analog ports require an explicit lane representation and remain a capability gap; do not replace a required pin with a sibling, unrelated connector, or whole-circuit claim; observation identities are not automatically valid operating-condition or coverage targets",
			"participant_port observations and generated port:<id> supply sources require a supply reference_domain naming the actual return, or exactly one circuit reference; ground names are not routing evidence; reference-domain declarations use empty reference_domain",
