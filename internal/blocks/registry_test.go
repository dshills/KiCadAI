...
func TestBuiltinRegistryListsInitialBlocksSorted(t *testing.T) {
	...
	want := []string{
		...
		"speaker_opamp_driver",
		"speaker_output_protection",
		"usb_c_power",
		"voltage_regulator",
	}
...
func TestBuiltinPlaceholdersHaveMetadata(t *testing.T) {
	for _, definition := range BuiltinDefinitions() {
		...
		structuralBlocks := map[string]bool{
			...
			"class_a_voltage_stage":        true,
			"class_ab_output_pair":         true,
			"class_ab_output_stage":        true,
			"class_ab_speaker_power_stage": true,
			...
			"speaker_opamp_driver":         true,
			"speaker_output_protection":    true,
