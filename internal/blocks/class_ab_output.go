...
func classABOutputStagePCBRealization() *PCBRealization {
	return &PCBRealization{
		...
		PlacementGroups: []PCBPlacementGroup{
			{ID: "bias_string", ComponentRoles: []string{"upper_bias_feed", "bias_upper", "bias_lower", "lower_bias_feed"}, AnchorRole: "bias_upper", Bounds: &RelativeBounds{MinXMM: 0, MinYMM: -5, MaxXMM: 9, MaxYMM: 5}, Description: "Keep the diode bias string thermally adjacent to the output pair."},
		...
		Constraints: []PCBConstraint{
			{ID: "upper_bias_thermal_coupling", Kind: "max_spacing", AppliesTo: []string{"bias_upper", "upper_output"}, MaxLengthMM: 5.2, Description: "Upper bias diode must stay thermally near the NPN output device while preserving package clearance; production thermal tracking remains review-required."},
			{ID: "lower_bias_thermal_coupling", Kind: "max_spacing", AppliesTo: []string{"bias_lower", "lower_output"}, MaxLengthMM: 5.2, Description: "Lower bias diode must stay thermally near the PNP output device while preserving package clearance; production thermal tracking remains review-required."},
