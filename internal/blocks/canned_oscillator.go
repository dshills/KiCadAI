...
func cannedOscillatorPCBRealization() *PCBRealization {
	return &PCBRealization{
		...
		TimingFixtures: []PCBTimingFixture{{
			...
			Kind:                    PCBTimingKindOscillator,
			SourceRole:              "oscillator",
			DecouplingRoles:         []string{"decoupling"},
			EnableControlRoles:      []string{"enable_pullup"},
			GroundNetTemplate:       "gnd",
			ClockNetTemplates:       []string{"clk_out"},
			LocalRouteIDs:           []string{"osc_vcc_decoupling", "osc_gnd_decoupling", "osc_enable_pullup", "osc_enable_vcc"},
			MaxDecouplingDistanceMM: timingMM(5),
			MinNoiseKeepoutMM:       timingMM(5),
			PreferredLayer:          "F.Cu",
			Roles:                   map[string]PCBTimingRole{"oscillator": PCBTimingRoleOscillator, "decoupling": PCBTimingRoleDecoupling, "enable_pullup": PCBTimingRoleEnableControl},
			Description:             "Keep the packaged oscillator supply bypass and enable pull-up close to the clock source.",
		}},
		Constraints: []PCBConstraint{
			{ID: "oscillator_decoupling_proximity", Kind: "proximity", NetTemplate: "vcc", AppliesTo: []string{"oscillator", "decoupling"}, MaxLengthMM: 5, Description: "Place oscillator decoupling close to the package supply pins."},
