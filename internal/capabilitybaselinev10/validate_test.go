...
func TestBuildValidatesAllFourOutcomesReplayGatesAndPromotions(t *testing.T) {
	...
	if report.CaseCount != 24 || report.EnvironmentSHA256 != strings.Repeat("b", 64) || report.EvaluatorManifestSHA256 != strings.Repeat("c", 64) ||
		...
		t.Fatalf("unexpected V10 baseline report: %+v", report)
	}
...
func TestValidateRejectsV9IdentityAndTamperedHashes(t *testing.T) {
	records := baselineRecords()
	records[0].Case.ID = "v9_case_001"
	if _, err := Build(strings.Repeat("a", 64), records); !errors.Is(err, ErrInvalidEvidence) {
		t.Fatalf("V9 identity error = %v", err)
	}
	report, err := Build(strings.Repeat("a", 64), baselineRecords())
	if err != nil {
		t.Fatal(err)
...
func baselineRecords() []CaseEvidence {
	...
	for index := 1; index <= 24; index++ {
		id := "v10_case_" + pad3(index)
		outcome := outcomeOrder[(index-1)%len(outcomeOrder)]
		current := capabilityroundsv10.Case{ID: id, Role: "discovery", ReportingDomain: domains[(index-1)%len(domains)],
		...
		if outcome == "pass" {
			...
			leaf := capabilityroundsv10.Leaf{Stage: stage, Category: stage, Scope: "scope_" + pad3(index), Capability: "capability_" + pad3(index),
				Code: "CODE_" + pad3(index), RequiredEvidence: []string{"diagnostic_evidence"}}
			current.Frontier = []capabilityroundsv10.Gap{{ObligationAnchor: digest("anchor-" + id), Path: []capabilityroundsv10.Leaf{leaf}, Diagnostics: []string{"diagnostic"}}}
		}
...
func pad3(value int) string { return fmt.Sprintf("%03d", value) }
