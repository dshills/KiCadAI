...
func TestFunctionalPinAwareSerializedReplay(t *testing.T) {
	root, cli := os.Getenv("KICADAI_PIN_AWARE_REPLAY_ARTIFACTS"), os.Getenv("KICADAI_KICAD_CLI")
	if root == "" || cli == "" {
	...
	for _, controller := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			promotion, issues := SynthesizeClosedLoop(ctx, r, search, ArchitectureSimulationPlanResolver{GraphResolver: resolver, ProvenanceRegistry: provenance, FunctionalLayoutProfile: schematiclayout.FunctionalOwnershipV3}, modelHash, nil, closedloopsynthesis.DefaultPolicy())
