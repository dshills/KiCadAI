...
func TestFunctionalJointRecordedLayoutDiagnostic(t *testing.T) {
	root, out := os.Getenv("KICADAI_JOINT_DIAGNOSTIC_INPUT"), os.Getenv("KICADAI_JOINT_DIAGNOSTIC_TRANSACTION")
	if root == "" || out == "" {
	...
	request.ExplicitCircuit.Schematic.Layout.FunctionalProfile = schematiclayout.FunctionalOwnershipV4
