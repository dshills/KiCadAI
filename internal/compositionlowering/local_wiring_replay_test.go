...
func TestFunctionalLocalWiringSerializedReplay(t *testing.T) {
	root, cli := os.Getenv("KICADAI_LOCAL_WIRING_REPLAY_ARTIFACTS"), os.Getenv("KICADAI_KICAD_CLI")
	if root == "" || cli == "" {
	...
	for _, controller := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			promotion, issues := SynthesizeClosedLoop(ctx, r, search, ArchitectureSimulationPlanResolver{GraphResolver: resolver, ProvenanceRegistry: provenance, FunctionalLayoutProfile: schematiclayout.FunctionalOwnershipV5}, modelHash, nil, closedloopsynthesis.DefaultPolicy())
