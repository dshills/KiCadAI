...
func TestFunctionalPowerLocalitySerializedReplay(t *testing.T) {
	root, cli := os.Getenv("KICADAI_POWER_LOCALITY_REPLAY_ARTIFACTS"), os.Getenv("KICADAI_KICAD_CLI")
	if root == "" || cli == "" {
	...
	resolver := circuitgraph.NewResolver(circuitgraph.ResolveOptions{Catalog: catalog, CatalogID: "checked-in", SchematicLayoutProfile: circuitgraph.SchematicLayoutTopologyV2})
	...
	for _, controller := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			promotion, issues := SynthesizeClosedLoop(ctx, r, search, ArchitectureSimulationPlanResolver{GraphResolver: resolver, ProvenanceRegistry: provenance, FunctionalLayoutProfile: schematiclayout.FunctionalOwnershipV6}, modelHash, nil, closedloopsynthesis.DefaultPolicy())
			...
			for _, run := range []string{"first", "second"} {
				...
				if run == "second" {
					encoded, err := json.Marshal(promotion.Request)
					if err != nil {
						t.Fatal(err)
					}
					...
					write(filepath.Join(dir, "replayed_workflow_request.json"), request)
				}
				...
				result := designworkflow.Create(ctx, request, opts)
				...
				files, err := filepath.Glob(filepath.Join(output, "*.kicad_sch"))
				if err != nil || len(files) != 1 {
					t.Fatal("expected exactly one native flat sheet", err)
				}
				readback, err := schematic.ReadFile(files[0])
				if err != nil {
					t.Fatal(err)
				}
				audit := designapi.AuditNativeAnnotations(readback)
				write(filepath.Join(dir, run+"_annotation_audit.json"), map[string]any{"schema": "kicadai.native-annotation-audit.v1", "schematic": files[0], "labels": len(readback.Labels), "symbols": len(readback.Symbols), "note_lines": len(readback.Texts) + len(readback.RawItems), "issues": audit})
				if len(audit) != 0 {
					t.Fatal(audit)
				}
			}
