...
func Publish(request Request) (Result, error) {
	...
	if err := capabilitybaselinev10.Validate(request.Report); err != nil {
		return Result{}, fmt.Errorf("validate V10 baseline report: %w", err)
	}
	if err := validateBinding(request.Binding); err != nil {
		return Result{}, fmt.Errorf("invalid V10 baseline publication binding: %w", err)
	}
	if request.Binding.CorpusManifestSHA256 != request.Report.CorpusManifestSHA256 {
		return Result{}, fmt.Errorf("V10 baseline corpus manifest binding does not match the report")
	}
	if request.Binding.EnvironmentSHA256 != request.Report.EnvironmentSHA256 {
		return Result{}, fmt.Errorf("V10 baseline environment binding does not match the report")
	}
	if request.Binding.EvaluatorManifestSHA256 != request.Report.EvaluatorManifestSHA256 {
		return Result{}, fmt.Errorf("V10 baseline evaluator manifest binding does not match the report")
	}

	reportBytes, err := capabilitybaselinev10.MarshalJSONStable(request.Report)
	if err != nil {
		return Result{}, err
	}
	...
	if err := atomicdir.Publish(destination, func(stage string) error {
		...
		return Result{}, fmt.Errorf("publish V10 discovery baseline: %w", err)
	}
	verified, err := Verify(destination)
	if err != nil {
		return Result{}, fmt.Errorf("verify published V10 discovery baseline: %w", err)
	}
	if verified.Manifest.Hash != manifest.Hash {
		return Result{}, fmt.Errorf("published V10 discovery baseline commitment drifted")
	}
