...
func Verify(root string) (Verification, error) {
	...
	if err := decodeStrict(manifestBytes, &manifest); err != nil {
		return Verification{}, fmt.Errorf("decode V10 baseline manifest: %w", err)
	}
	...
	if err != nil || !bytes.Equal(manifestBytes, wantManifestBytes) {
		return Verification{}, fmt.Errorf("V10 baseline manifest is not canonical")
	}
	...
	if err := decodeStrict(reportBytes, &report); err != nil {
		return Verification{}, fmt.Errorf("decode V10 baseline report: %w", err)
	}
	if err := capabilitybaselinev10.Validate(report); err != nil {
		return Verification{}, fmt.Errorf("validate V10 baseline report: %w", err)
	}
	wantReportBytes, err := capabilitybaselinev10.MarshalJSONStable(report)
	if err != nil {
		return Verification{}, err
	}
	...
	if !bytes.Equal(reportBytes, wantReportBytes) || hashBytes(reportBytes) != manifest.ReportSHA256 ||
		...
		return Verification{}, fmt.Errorf("V10 baseline report does not match its manifest")
	}
	if len(report.Cases) != len(manifest.Cases) {
		return Verification{}, fmt.Errorf("V10 baseline report case count does not match its manifest")
	}
	...
	for index, reference := range manifest.Cases {
		...
		if hashBytes(data) != reference.SHA256 {
			return Verification{}, fmt.Errorf("V10 case %s hash mismatch", reference.ID)
		}
		var record capabilitybaselinev10.CaseEvidence
		if err := decodeStrict(data, &record); err != nil {
			return Verification{}, fmt.Errorf("decode V10 case %s: %w", reference.ID, err)
		}
		canonical, err := canonicalJSON(record)
		if err != nil || !bytes.Equal(data, canonical) || !reflect.DeepEqual(record, report.Cases[index]) {
			return Verification{}, fmt.Errorf("V10 case %s does not reproduce report evidence", reference.ID)
		}
	...
	if err != nil || !bytes.Equal(audit, auditBytes(manifest, hashBytes(manifestBytes))) {
		return Verification{}, fmt.Errorf("V10 baseline audit does not reproduce")
	}
	...
	if err != nil || !bytes.Equal(checksums, checksumBytes(expectedFiles)) {
		return Verification{}, fmt.Errorf("V10 baseline checksums do not reproduce")
	}
...
func validateManifest(value Manifest) error {
	if err := validateBinding(value.Binding); err != nil {
		return fmt.Errorf("invalid V10 baseline publication binding: %w", err)
	}
	if value.Schema != ManifestSchema || value.Version != Version || value.CaseCount != ExpectedCases ||
		...
		return fmt.Errorf("invalid V10 baseline publication manifest")
	}
	total := 0
	for index, count := range value.OutcomeCounts {
		if count.Outcome != frozenOutcomeOrder[index] || count.Count < 0 {
			return fmt.Errorf("invalid V10 baseline outcome counts")
		}
		total += count.Count
	}
	if total != ExpectedCases {
		return fmt.Errorf("invalid V10 baseline outcome total")
	}
	for index, reference := range value.Cases {
		...
		if reference.ID != wantID || reference.Path != CaseDirectory+"/"+wantID+".json" || !digestPattern.MatchString(reference.SHA256) {
			return fmt.Errorf("invalid V10 baseline case reference %d", index)
		}
	}
	wantHash, err := manifestHash(value)
	if err != nil {
		return fmt.Errorf("invalid V10 baseline manifest commitment: %w", err)
	}
	if wantHash != value.Hash {
		return fmt.Errorf("invalid V10 baseline manifest commitment")
	}
	return nil
}
...
func readRegularFile(root, relative string) ([]byte, error) {
	if relative == "" || path.IsAbs(relative) || filepath.IsAbs(relative) || path.Clean(relative) != relative ||
		...
		return nil, fmt.Errorf("invalid V10 baseline artifact path %q", relative)
	}
	...
	if !pathInfo.Mode().IsRegular() || pathInfo.Size() > maximumArtifactBytes {
		return nil, fmt.Errorf("V10 baseline artifact %s is not a bounded regular file", relative)
	}
	...
	if !openInfo.Mode().IsRegular() || openInfo.Size() > maximumArtifactBytes || !os.SameFile(pathInfo, openInfo) {
		return nil, fmt.Errorf("V10 baseline artifact %s changed during verification", relative)
	}
	...
	if len(data) > maximumArtifactBytes {
		return nil, fmt.Errorf("V10 baseline artifact %s exceeds its size bound", relative)
	}
	return data, nil
}
...
func verifyExactFiles(root string, expected map[string][]byte) error {
	...
	if err != nil || !rootInfo.IsDir() || rootInfo.Mode()&os.ModeSymlink != 0 {
		return fmt.Errorf("V10 baseline root is not a real directory")
	}
	if err := filepath.WalkDir(root, func(path string, entry fs.DirEntry, walkErr error) error {
		...
		if relative == "." || relative == CaseDirectory {
			if !entry.IsDir() {
				return fmt.Errorf("V10 baseline directory shape is invalid")
			}
			return nil
		}
		if entry.IsDir() || entry.Type()&os.ModeSymlink != 0 || !entry.Type().IsRegular() {
			return fmt.Errorf("unexpected V10 baseline path %s", relative)
		}
		if _, exists := want[relative]; !exists || want[relative] {
			return fmt.Errorf("unexpected or duplicate V10 baseline file %s", relative)
		}
		want[relative] = true
		return nil
	}); err != nil {
	...
	for path, seen := range want {
		if !seen {
			return fmt.Errorf("missing V10 baseline file %s", path)
		}
	}
	return nil
}
