...
func LoadPacket(root string, policy Policy) (Packet, error) {
	...
	if packetSetHash != policy.PacketSetSHA256 {
		return Packet{}, fmt.Errorf("packet set does not match frozen V10 policy")
	}
...
func readRegularFileUnder(root, relative string) ([]byte, error) {
	...
	current := root
	for _, slashSegment := range strings.Split(relative, "/") {
		if slashSegment == "" || slashSegment == "." || slashSegment == ".." {
			return nil, fmt.Errorf("unsafe relative path segment")
		}
		segment := filepath.FromSlash(slashSegment)
	...
	if len(data) > maximumPacketFileBytes {
		return nil, fmt.Errorf("file exceeds maximum allowed size of %d bytes", maximumPacketFileBytes)
	}
	return data, nil
}
