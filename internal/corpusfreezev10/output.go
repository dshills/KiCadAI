...
func (report Report) MarshalJSONStable() ([]byte, error) {
	if report.Schema != "kicadai.behavior-corpus-validation-report.v10" || report.Version != 10 ||
		...
		return nil, fmt.Errorf("V10 validation report is incomplete")
	}
	for _, entry := range report.Entries {
		if strings.TrimSpace(entry.ID) == "" || strings.TrimSpace(entry.CircuitRole) == "" ||
			...
			return nil, fmt.Errorf("V10 validation report entry is invalid")
		}
	}
