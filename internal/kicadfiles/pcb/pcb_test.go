...
func TestCorrectnessFixturePCBExercisesCoreObjects(t *testing.T) {
	...
	if len(board.Footprints) != 3 {
		t.Fatalf("Footprints = %d, want 3", len(board.Footprints))
	}
	if len(board.Tracks) == 0 || len(board.TrackArcs) == 0 || len(board.Vias) == 0 || len(board.Zones) == 0 {
		t.Fatalf("fixture missing routed objects: tracks=%d arcs=%d vias=%d zones=%d", len(board.Tracks), len(board.TrackArcs), len(board.Vias), len(board.Zones))
	}
	var hasSMD, hasThroughHole bool
	for _, footprint := range board.Footprints {
		if len(footprint.Graphics) == 0 {
			t.Fatalf("%s missing footprint graphics", footprint.Reference)
		}
	...
	if !hasSMD || !hasThroughHole {
		t.Fatalf("fixture pad coverage smd=%v through-hole=%v", hasSMD, hasThroughHole)
	}
}

func TestCheckedInCorrectnessFixtureMatchesGenerator(t *testing.T) {
	board, err := CorrectnessFixturePCB(CorrectnessFixtureInput{
		DesignID: kicadfiles.UUID("12345678-1234-5678-9234-123456789abc"),
		Seed:     "pcb-object-correctness",
	})
	...
	if err := Write(&generated, board); err != nil {
		t.Fatalf("Write returned error: %v", err)
	}

	path := repoPathForPCBTest(t, "examples", "08_pcb_object_correctness", "pcb_object_correctness.kicad_pcb")
	if os.Getenv("KICADAI_UPDATE_FIXTURES") == "1" {
	...
	if !bytes.Equal(normalizeLineEndings(existing), normalizeLineEndings(generated.Bytes())) {
		t.Fatalf("checked-in fixture is stale; run KICADAI_UPDATE_FIXTURES=1 go test ./internal/kicadfiles/pcb -run TestCheckedInCorrectnessFixtureMatchesGenerator")
	}
}
