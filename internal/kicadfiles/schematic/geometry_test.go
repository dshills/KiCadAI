...
func TestTransformConnectionAnchorUsesKiCadMirrorThenRotateOrder(t *testing.T) {
	...
	if got := TransformConnectionAnchor(offset, 90, SymbolMirrorNone); got != (kicadfiles.Point{X: kicadfiles.MM(4), Y: kicadfiles.MM(-3)}) {
		t.Fatalf("clockwise 90 degree rotation = %#v", got)
	}
	if got := TransformConnectionAnchor(offset, 270, SymbolMirrorNone); got != (kicadfiles.Point{X: kicadfiles.MM(-4), Y: kicadfiles.MM(3)}) {
		t.Fatalf("clockwise 270 degree rotation = %#v", got)
	}
