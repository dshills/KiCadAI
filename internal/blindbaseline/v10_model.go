...
const (
	ManifestSchemaV10  = "kicadai.closed-loop-open-set-held-out-baseline-seal.v10"
	ManifestVersionV10 = 10
	AlgorithmV10       = "AES-256-GCM/v10-record-set/random-unique-nonce-per-record/length-delimited-aad"
	CipherFileV10      = "held_out_baseline.records.sealed"
	nonceBytesV10      = 12
	expectedCasesV10   = 24
)

// V10Binding commits every public input that may affect the generation-zero
// held-out baseline. It intentionally contains no held-out case, outcome,
// frontier, path, diagnostic, or timing data.
type V10Binding struct {
...
func (binding V10Binding) fields() []bindingField {
	return []bindingField{
...
// V10Manifest is the complete non-revealing public commitment to one frozen
// 24-record blind baseline publication.
type V10Manifest struct {
	Schema                   string     `json:"schema"`
	Version                  int        `json:"version"`
	Algorithm                string     `json:"algorithm"`
	Binding                  V10Binding `json:"binding"`
...
// V10Request supplies opaque per-case evidence and the complete public binding
// to the isolated publisher. Records must already be in frozen corpus order.
type V10Request struct {
...
// V10Result reports the exact manifest published by PublishV10.
type V10Result struct {
	Manifest V10Manifest
}
