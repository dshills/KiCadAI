...
func TestV6PublishVerifyOpenAndVersionSeparation(t *testing.T) {
	repository := t.TempDir()
	external := t.TempDir()
	destination := filepath.Join(repository, "sealed")
	keyPath := filepath.Join(external, "baseline.key")
	payload := []byte(`{"sensitive":"v6-held-out"}`)
	result, err := PublishV6(V6Request{
		RepositoryRoot: repository, DestinationRoot: destination, KeyPath: keyPath,
		ReservedKeyPaths: []string{filepath.Join(external, "source.key")}, Binding: testV6Binding(),
		Payload: payload, CaseCount: 18, Random: bytes.NewReader(bytes.Repeat([]byte{0x42}, 128)),
	})
	if err != nil {
		t.Fatal(err)
	}
	manifest, err := VerifyV6(destination)
	if err != nil || manifest != result.Manifest {
		t.Fatalf("verify V6 manifest = %#v, %v", manifest, err)
	}
	...
	opened, err := OpenV6(key, manifest, ciphertext)
	if err != nil || !bytes.Equal(opened, payload) {
		t.Fatalf("opened V6 payload = %q, %v", opened, err)
	}
	if _, err := Verify(destination); err == nil {
		t.Fatal("V5 verifier accepted V6 publication")
	}
	...
	v6Manifest := manifest
	v6Manifest.CiphertextSHA256 = hashBytes(v5Ciphertext)
	v6Manifest.Hash = hashBytes(manifestCommitmentV6(v6Manifest))
	if _, err := OpenV6(key, v6Manifest, v5Ciphertext); err == nil {
		t.Fatal("V6 opener accepted V5 ciphertext")
	}
}

func TestV6AuthenticationRejectsMutationAndNoReplaceCleansKey(t *testing.T) {
	repository := t.TempDir()
	external := t.TempDir()
	destination := filepath.Join(repository, "sealed")
	keyPath := filepath.Join(external, "baseline.key")
	result, err := PublishV6(V6Request{RepositoryRoot: repository, DestinationRoot: destination, KeyPath: keyPath, Binding: testV6Binding(), Payload: []byte("sensitive"), CaseCount: 18, Random: bytes.NewReader(bytes.Repeat([]byte{0x42}, 128))})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := OpenV6(key, result.Manifest, mutated); err == nil {
		t.Fatal("V6 opener accepted ciphertext mutation")
	}
	manifest := result.Manifest
	manifest.Binding.SelectionSHA256 = testHash("f")
	manifest.Hash = hashBytes(manifestCommitmentV6(manifest))
	if _, err := OpenV6(key, manifest, ciphertext); err == nil {
		t.Fatal("V6 opener accepted authenticated binding mutation")
	}
	secondKey := filepath.Join(external, "second.key")
	if _, err := PublishV6(V6Request{RepositoryRoot: repository, DestinationRoot: destination, KeyPath: secondKey, Binding: testV6Binding(), Payload: []byte("sensitive"), CaseCount: 18, Random: bytes.NewReader(bytes.Repeat([]byte{0x43}, 128))}); err == nil {
		t.Fatal("V6 publication overwrote its destination")
	}
	if _, err := os.Stat(secondKey); !os.IsNotExist(err) {
		t.Fatal("failed V6 no-replace publication retained its new key")
	}
}

func TestV6BindingFieldRegistryCoversJSONContractInOrder(t *testing.T) {
	typeOfBinding := reflect.TypeOf(V6Binding{})
	fields := (V6Binding{}).fields()
	if len(fields) != typeOfBinding.NumField() {
		t.Fatalf("V6 binding registry fields = %d, struct fields = %d", len(fields), typeOfBinding.NumField())
	}
	seen := map[string]bool{}
	for index := range fields {
		name := strings.Split(typeOfBinding.Field(index).Tag.Get("json"), ",")[0]
		if name == "" || name != fields[index].name || seen[name] {
			t.Fatalf("V6 binding registry entry %d does not match unique JSON field %q", index, name)
		}
		seen[name] = true
	}
}

func testV6Binding() V6Binding {
	return V6Binding{
