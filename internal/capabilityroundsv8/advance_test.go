...
func TestEvaluateRoundAdmitsUpliftAndAppendOnlySuccessors(t *testing.T) {
	...
	if evaluation.Status != EvaluationPublicAdmitted || evaluation.DiscoveryPassBefore != 14 || evaluation.DiscoveryPassAfter != 15 || evaluation.NewActiveCohortPasses != 1 || len(evaluation.AdvancedCaseIDs) != 4 || len(evaluation.AdvancedReportingDomains) != 4 || len(evaluation.AdvancedCircuitRoles) != 4 || len(evaluation.Successors) != 3 {
...
func TestEvaluateRoundFailsClosedOnFanoutDriftRegressionAndBudget(t *testing.T) {
	...
	state := RoundState{Generation: 2, UsedAtomCount: 7, UsedMemberCount: 25, PriorAtomKeys: priorAtomsExceptSelected(t, selected, 7), ActiveCohortIDs: initialState().ActiveCohortIDs}
