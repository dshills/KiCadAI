...
func TestPlaceContinuesPastInvalidCandidateRotation(t *testing.T) {
	...
	result, ok, _ := placeComponent(req.Components[0], req, newOccupancy(req), nil, padsByRef, componentRotatedPadMaps(req.Components, padsByRef), netsByComponent(req.Nets), advancedNetMembershipByRef(req.Nets), keepTogetherPeersByComponent(req), nil)
	if !ok {
