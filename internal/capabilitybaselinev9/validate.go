...
var ErrInvalidEvidence = errors.New("invalid V9 generation-zero evidence")
...
func Build(corpusManifestSHA256 string, records []CaseEvidence) (Report, error) {
	if !digestPattern.MatchString(corpusManifestSHA256) || len(records) != capabilityroundsv9.FrozenPolicy().ExpectedDiscoveryCases {
...
func Validate(report Report) error {
	if report.Schema != ReportSchema || report.Version != Version || report.CaseCount != capabilityroundsv9.FrozenPolicy().ExpectedDiscoveryCases ||
...
func validateFrontier(value capabilityroundsv9.Case, policy capabilityroundsv9.Policy) error {
	...
	for _, gap := range value.Frontier {
		...
		pathHash, err := capabilityroundsv9.PathHash(gap)
		if err != nil {
