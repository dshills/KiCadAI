...
func TestRunV20UsesFrozenSerialTwoReplayTransport(t *testing.T) {
	...
	request := testRequest(t, filepath.Join(t.TempDir(), "v20"))
	report, err := executor.RunV20(context.Background(), request)
	if err != nil || report.Hash == "" || maximum.Load() != 1 || calls.Load() != 48 {
		t.Fatalf("V20 transport = hash=%q maximum=%d calls=%d error=%v", report.Hash, maximum.Load(), calls.Load(), err)
	}
}
