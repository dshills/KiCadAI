...
const v11CleanRootSchema = "kicadai.closed-loop-open-set-clean-root.v11"
const v11EvaluationRootSchema = "kicadai.closed-loop-open-set-evaluation-root.v11"
const v11EvaluationRootMarkerName = "V11_EVALUATION_ROOT.json"

type caseResultV11 struct {
	evidence     capabilitybaselinev10.CaseEvidence
	replaySpools []string
}

// RunV11 executes the V11 memory-bounded correction while preserving the V10
// corpus, synthesis policy, evidence schema, and four-case concurrency limit.
// It is separate from Run so the frozen V10 evaluator remains byte-verifiable.
func (executor Executor) RunV11(ctx context.Context, request Request) (capabilitybaselinev10.Report, error) {
	if executor.decode == nil || executor.synthesize == nil || executor.promote == nil || executor.observe == nil {
		return capabilitybaselinev10.Report{}, fmt.Errorf("V11 evaluator engine is incomplete")
	}
	...
	if !digestPattern.MatchString(request.CorpusManifestSHA256) || len(request.Cases) != 24 {
		return capabilitybaselinev10.Report{}, fmt.Errorf("V11 corpus binding or discovery cohort is invalid")
	}
	for index, input := range request.Cases {
		if wantID := fmt.Sprintf("v10_case_%03d", index+1); input.Entry.ID != wantID {
			return capabilitybaselinev10.Report{}, fmt.Errorf("V11 reused-corpus case order differs at %d", index)
		}
	}
	marker := evaluationRootMarker{
		Schema: v11EvaluationRootSchema, Version: 11,
		...
		CaseCount:               len(request.Cases), ReplaysPerCase: 2, ParallelCaseLimit: ParallelCaseLimit,
	}
	if err := prepareV11OutputRoot(request.OutputRoot, request.Resume, marker); err != nil {
		return capabilitybaselinev10.Report{}, err
	}
	records, completed, err := loadV11CaseCheckpoints(request.OutputRoot, request.Cases, marker, request.Resume)
	if err != nil {
		return capabilitybaselinev10.Report{}, err
	}
	...
	for worker := 0; worker < ParallelCaseLimit; worker++ {
		...
		go func() {
			...
			for index := range jobs {
				result, runErr := executor.runCaseV11(ctx, request.OutputRoot, request.CorpusManifestSHA256, request.Cases[index], request.Environment, environmentSHA256)
				if runErr == nil {
...
func (executor Executor) runCaseV11(ctx context.Context, outputRoot, corpusManifestSHA256 string, input CaseInput, environment Environment, environmentSHA256 string) (caseResultV11, error) {
	...
	for replay := 1; replay <= 2; replay++ {
		...
		commitment, err := prepareCleanRoot(root, cleanRootMarker{
			Schema: v11CleanRootSchema, Version: 11, CaseID: input.Entry.ID, Replay: replay,
...
func prepareV11OutputRoot(root string, resume bool, marker evaluationRootMarker) error {
	if strings.TrimSpace(root) == "" {
		return fmt.Errorf("V11 evaluator output root is required")
	}
	clean := filepath.Clean(root)
	if resume {
		info, err := os.Lstat(clean)
		if err != nil || !info.IsDir() || info.Mode()&os.ModeSymlink != 0 {
			return fmt.Errorf("resume V11 evaluator output root: not a real directory")
		}
		...
		if err != nil || !checkpointInfo.IsDir() || checkpointInfo.Mode()&os.ModeSymlink != 0 {
			return fmt.Errorf("resume V11 evaluator checkpoint root: not a real directory")
		}
		data, err := readBoundedRegularFile(filepath.Join(clean, v11EvaluationRootMarkerName), 64*1024)
		if err != nil {
			return fmt.Errorf("resume V11 evaluator root commitment: %w", err)
		}
		var actual evaluationRootMarker
		if err := decodeStrictJSON(data, &actual); err != nil || !reflect.DeepEqual(actual, marker) {
			return fmt.Errorf("resume V11 evaluator root commitment differs")
		}
		return nil
	}
	...
	if err := os.Mkdir(clean, 0o755); err != nil {
		return fmt.Errorf("create fresh V11 evaluator output root: %w", err)
	}
	if err := os.Mkdir(filepath.Join(clean, checkpointDirectoryName), 0o755); err != nil {
		return fmt.Errorf("create V11 evaluator checkpoint root: %w", err)
	}
	data, err := json.Marshal(marker)
	if err != nil {
		return err
	}
	return writeNewReadOnlyFile(filepath.Join(clean, v11EvaluationRootMarkerName), data)
}

func loadV11CaseCheckpoints(root string, cases []CaseInput, marker evaluationRootMarker, resume bool) ([]capabilitybaselinev10.CaseEvidence, []bool, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("read V11 evaluator checkpoints: %w", err)
	}
	...
	for _, entry := range entries {
		...
		if strings.HasPrefix(entry.Name(), ".v10-checkpoint-") && entry.Type().IsRegular() {
			if err := os.Remove(path); err != nil {
				return nil, nil, fmt.Errorf("remove incomplete V11 checkpoint: %w", err)
			}
			continue
		}
		return nil, nil, fmt.Errorf("unexpected V11 evaluator checkpoint entry %q", entry.Name())
	}
	for index, input := range cases {
		...
		if errors.Is(err, os.ErrNotExist) {
			if err := os.RemoveAll(filepath.Join(root, input.Entry.ID)); err != nil {
				return nil, nil, fmt.Errorf("remove incomplete V11 case root: %w", err)
			}
			continue
		}
		if err != nil {
		...
		if err := validateV11CheckpointRoots(root, input, marker, record); err != nil {
			return nil, nil, err
		}
...
func validateV11CheckpointRoots(root string, input CaseInput, marker evaluationRootMarker, record capabilitybaselinev10.CaseEvidence) error {
	for replay := 1; replay <= 2; replay++ {
		...
		if err != nil || hashBytes(data) != record.ReplayRootSHA256[replay-1] {
			return fmt.Errorf("%s replay %d V11 checkpoint root is invalid", input.Entry.ID, replay)
		}
		var actual cleanRootMarker
		if err := decodeStrictJSON(data, &actual); err != nil {
			return fmt.Errorf("%s replay %d V11 checkpoint root is malformed", input.Entry.ID, replay)
		}
		...
		if !reflect.DeepEqual(actual, expected) {
			return fmt.Errorf("%s replay %d V11 checkpoint root binding differs", input.Entry.ID, replay)
		}
	}
	return nil
}
