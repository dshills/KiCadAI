...
const v15CleanRootSchema = "kicadai.closed-loop-open-set-clean-root.v15"
const v15EvaluationRootSchema = "kicadai.closed-loop-open-set-evaluation-root.v15"
const v15EvaluationRootMarkerName = "V15_EVALUATION_ROOT.json"
const v15ParallelCaseLimit = 1

func releaseReplayMemoryV15() {
	runtime.GC()
	debug.FreeOSMemory()
}

type replayResultV15 struct {
	rootSHA256   string
	replaySHA256 string
	spoolPath    string
...
// RunV15 executes one case and one replay at a time. Each replay is isolated in
...
func (executor Executor) RunV15(ctx context.Context, request Request) (capabilitybaselinev10.Report, error) {
	return executor.runV15(ctx, request, releaseReplayMemoryV15)
}

func (executor Executor) runV15(ctx context.Context, request Request, releaseReplayMemory func()) (capabilitybaselinev10.Report, error) {
	if executor.decode == nil || executor.synthesize == nil || executor.promote == nil || executor.observe == nil {
		return capabilitybaselinev10.Report{}, fmt.Errorf("V15 evaluator engine is incomplete")
	}
	if releaseReplayMemory == nil {
		return capabilitybaselinev10.Report{}, fmt.Errorf("V15 replay memory release is required")
	}
	...
	if !digestPattern.MatchString(request.CorpusManifestSHA256) || len(request.Cases) != 24 {
		return capabilitybaselinev10.Report{}, fmt.Errorf("V15 corpus binding or discovery cohort is invalid")
	}
	for index, input := range request.Cases {
		if wantID := fmt.Sprintf("v10_case_%03d", index+1); input.Entry.ID != wantID {
			return capabilitybaselinev10.Report{}, fmt.Errorf("V15 reused-corpus case order differs at %d", index)
		}
	}
	marker := evaluationRootMarker{
		Schema: v15EvaluationRootSchema, Version: 15,
		...
		CaseCount:               len(request.Cases), ReplaysPerCase: 2, ParallelCaseLimit: v15ParallelCaseLimit,
	}
	if err := prepareV15OutputRoot(request.OutputRoot, request.Resume, marker); err != nil {
		return capabilitybaselinev10.Report{}, err
	}
	records, completed, err := loadV15CaseCheckpoints(request.OutputRoot, request.Cases, marker, request.Resume)
	if err != nil {
		return capabilitybaselinev10.Report{}, err
	}
	for index, input := range request.Cases {
		...
		result, runErr := executor.runCaseV15(ctx, request.OutputRoot, request.CorpusManifestSHA256, input, request.Environment, environmentSHA256, releaseReplayMemory)
		if runErr == nil {
...
func (executor Executor) runCaseV15(ctx context.Context, outputRoot, corpusManifestSHA256 string, input CaseInput, environment Environment, environmentSHA256 string, releaseReplayMemory func()) (caseResultV11, error) {
	...
	replays := make([]replayResultV15, 0, 2)
	for replay := 1; replay <= 2; replay++ {
		result, runErr := executor.runReplayV15(ctx, caseRoot, corpusManifestSHA256, input, requirement, environment, environmentSHA256, replay)
		releaseReplayMemory()
		if runErr != nil {
...
func (executor Executor) runReplayV15(ctx context.Context, caseRoot, corpusManifestSHA256 string, input CaseInput, requirement opentopologysynthesis.Requirement, environment Environment, environmentSHA256 string, replay int) (replayResultV15, error) {
	root := filepath.Join(caseRoot, fmt.Sprintf("replay-%d", replay))
	commitment, err := prepareCleanRoot(root, cleanRootMarker{
		Schema: v15CleanRootSchema, Version: 15, CaseID: input.Entry.ID, Replay: replay,
		CorpusManifestSHA256: corpusManifestSHA256, RequirementSHA256: input.Entry.RequirementSHA256,
		EnvironmentSHA256: environmentSHA256, EvaluatorManifestSHA256: environment.EvaluatorManifestSHA256,
	})
	if err != nil {
		return replayResultV15{}, err
	}
	...
	if run.Report.Status == opentopologysynthesis.StatusInvalid || run.Report.Status == opentopologysynthesis.StatusCanceled {
		return replayResultV15{}, fmt.Errorf("synthesis terminated with non-capability status %q", run.Report.Status)
	}
	spoolPath := filepath.Join(root, replaySpoolNameV11)
	digest, err := writeReplaySpoolV11(spoolPath, &run)
	if err != nil {
		return replayResultV15{}, fmt.Errorf("stream synthesis replay: %w", err)
	}
	result := replayResultV15{rootSHA256: commitment, replaySHA256: digest, spoolPath: spoolPath}
	...
	if replay == 1 {
		...
		if err != nil {
			return replayResultV15{}, fmt.Errorf("observe capability evidence: %w", err)
		}
...
func prepareV15OutputRoot(root string, resume bool, marker evaluationRootMarker) error {
	if strings.TrimSpace(root) == "" {
		return fmt.Errorf("V15 evaluator output root is required")
	}
	clean := filepath.Clean(root)
	if resume {
		info, err := os.Lstat(clean)
		if err != nil || !info.IsDir() || info.Mode()&os.ModeSymlink != 0 {
			return fmt.Errorf("resume V15 evaluator output root: not a real directory")
		}
		...
		if err != nil || !checkpointInfo.IsDir() || checkpointInfo.Mode()&os.ModeSymlink != 0 {
			return fmt.Errorf("resume V15 evaluator checkpoint root: not a real directory")
		}
		data, err := readBoundedRegularFile(filepath.Join(clean, v15EvaluationRootMarkerName), 64*1024)
		if err != nil {
			return fmt.Errorf("resume V15 evaluator root commitment: %w", err)
		}
		var actual evaluationRootMarker
		if err := decodeStrictJSON(data, &actual); err != nil || !reflect.DeepEqual(actual, marker) {
			return fmt.Errorf("resume V15 evaluator root commitment differs")
		}
		return nil
	}
	...
	if err := os.Mkdir(clean, 0o755); err != nil {
		return fmt.Errorf("create fresh V15 evaluator output root: %w", err)
	}
	if err := os.Mkdir(filepath.Join(clean, checkpointDirectoryName), 0o755); err != nil {
		return fmt.Errorf("create V15 evaluator checkpoint root: %w", err)
	}
	data, err := json.Marshal(marker)
	if err != nil {
		return err
	}
	return writeNewReadOnlyFile(filepath.Join(clean, v15EvaluationRootMarkerName), data)
}

func loadV15CaseCheckpoints(root string, cases []CaseInput, marker evaluationRootMarker, resume bool) ([]capabilitybaselinev10.CaseEvidence, []bool, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("read V15 evaluator checkpoints: %w", err)
	}
	...
	for _, entry := range entries {
		...
		if strings.HasPrefix(entry.Name(), ".v10-checkpoint-") && entry.Type().IsRegular() {
			if err := os.Remove(path); err != nil {
				return nil, nil, fmt.Errorf("remove incomplete V15 checkpoint: %w", err)
			}
			continue
		}
		return nil, nil, fmt.Errorf("unexpected V15 evaluator checkpoint entry %q", entry.Name())
	}
	for index, input := range cases {
		...
		if errors.Is(err, os.ErrNotExist) {
			if err := os.RemoveAll(filepath.Join(root, input.Entry.ID)); err != nil {
				return nil, nil, fmt.Errorf("remove incomplete V15 case root: %w", err)
			}
			continue
		}
		if err != nil {
		...
		if err := validateV15CheckpointRoots(root, input, marker, record); err != nil {
			return nil, nil, err
		}
...
func validateV15CheckpointRoots(root string, input CaseInput, marker evaluationRootMarker, record capabilitybaselinev10.CaseEvidence) error {
	for replay := 1; replay <= 2; replay++ {
		...
		if err != nil || hashBytes(data) != record.ReplayRootSHA256[replay-1] {
			return fmt.Errorf("%s replay %d V15 checkpoint root is invalid", input.Entry.ID, replay)
		}
		var actual cleanRootMarker
		if err := decodeStrictJSON(data, &actual); err != nil {
			return fmt.Errorf("%s replay %d V15 checkpoint root is malformed", input.Entry.ID, replay)
		}
		...
		if !reflect.DeepEqual(actual, expected) {
			return fmt.Errorf("%s replay %d V15 checkpoint root binding differs", input.Entry.ID, replay)
		}
	}
	return nil
}
