...
func TestRunV19UsesFrozenSerialTwoReplayTransport(t *testing.T) {
	...
	request := testRequest(t, filepath.Join(t.TempDir(), "v19"))
	report, err := executor.RunV19(context.Background(), request)
	if err != nil || report.Hash == "" || maximum.Load() != 1 || calls.Load() != 48 {
		t.Fatalf("V19 transport = hash=%q maximum=%d calls=%d error=%v", report.Hash, maximum.Load(), calls.Load(), err)
	}
}
