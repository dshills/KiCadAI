...
func TestRunV21UsesFrozenSerialTwoReplayTransport(t *testing.T) {
	...
	request := testRequest(t, filepath.Join(t.TempDir(), "v21"))
	report, err := executor.RunV21(context.Background(), request)
	if err != nil || report.Hash == "" || maximum.Load() != 1 || calls.Load() != 48 {
		t.Fatalf("V21 transport = hash=%q maximum=%d calls=%d error=%v", report.Hash, maximum.Load(), calls.Load(), err)
	}
}
