...
func TestRunV17SerializesCasesReleasesEveryReplayAndResumes(t *testing.T) {
	...
	request := testRequest(t, filepath.Join(t.TempDir(), "v17"))
	report, err := executor.runV17(context.Background(), request, func() { releaseCalls.Add(1) })
	if err != nil || report.Hash == "" || maximum.Load() != v17ParallelCaseLimit || synthesisCalls.Load() != 48 || releaseCalls.Load() != 48 {
		t.Fatalf("V17 run failed: hash=%q maximum=%d syntheses=%d releases=%d error=%v", report.Hash, maximum.Load(), synthesisCalls.Load(), releaseCalls.Load(), err)
	}
	data, err := os.ReadFile(filepath.Join(request.OutputRoot, v17EvaluationRootMarkerName))
	if err != nil {
		t.Fatal(err)
	}
	var marker evaluationRootMarker
	if err := json.Unmarshal(data, &marker); err != nil || marker.Schema != v17EvaluationRootSchema || marker.Version != 17 || marker.ParallelCaseLimit != v17ParallelCaseLimit {
		t.Fatalf("invalid V17 evaluation root: %+v, %v", marker, err)
	}
	...
	request.Resume = true
	resumed, err := resumeExecutor.runV17(context.Background(), request, func() { releaseCalls.Add(1) })
	if err != nil || resumed.Hash != report.Hash || resumeCalls.Load() != 0 {
		t.Fatalf("V17 authenticated resume failed: calls=%d hashes=%s/%s error=%v", resumeCalls.Load(), resumed.Hash, report.Hash, err)
	}
}
