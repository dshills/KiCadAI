...
The same native bytes and BOMs also match the fresh supported holdout cases: standard `fresh-01`, fast `fresh-02`, low_current `fresh-03`. Their unchanged `holdout-selection.json`, `holdout-validation.json`, `holdout-erc.json` and `holdout-drc.json` records are bound by the [holdout evidence manifest](../../specs/board-family-v1/evidence/acceptance/manifest.json). All ten fresh supported requests completed correctly. The rejected resize output is not an accepted example; that strict live suite remains failed. A [separately approved live follow-up](../../specs/board-family-v1/evidence/guardrails/assessment.json) subsequently passed four refusals and two clarifications without generating any board. It does not rewrite the earlier trial.
