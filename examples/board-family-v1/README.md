...
The same native bytes and BOMs also match the fresh supported holdout cases: standard `fresh-01`, fast `fresh-02`, low_current `fresh-03`. Their unchanged `holdout-selection.json`, `holdout-validation.json`, `holdout-erc.json` and `holdout-drc.json` records are bound by the [final evidence manifest](../../specs/board-family-v1/evidence/acceptance/manifest.json). All ten fresh supported requests completed correctly. The rejected resize output is not an accepted example; the overall strict live suite remains failed.
