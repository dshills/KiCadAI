...
const publication = "internal/capabilityfeedback/testdata/closed_loop_open_set_v22_public_1/report.json"
const predecessor = "internal/capabilityfeedback/testdata/closed_loop_open_set_v21_maintenance_1/report.json"
...
func TestV22Freeze(t *testing.T) {
	for _, path := range []string{"FREEZE.sha256", "EVALUATOR.sha256", "../V22_IMPLEMENTATION.sha256", "../EVIDENCE.sha256", "../DIAGNOSTIC.sha256", "../../generic-causal-topology-repair/V21_CONTRACT.sha256", "../../generic-causal-topology-repair/V21_EVALUATOR.sha256", "../../generic-analysis-model-solver-admission/V20_CONTRACT.sha256", "../../generic-analysis-model-solver-admission/V20_EVALUATOR.sha256"} {
		checkSeal(t, path)
	}
	var run struct {
	...
	if run.Schema != "kicadai.public-electrical-evaluation.v22" || run.Implementation != "a808d612769bcb4821a22cbf7dd6535feded1f6a" || run.GoVersion != "go1.26.8" || run.Platform != "darwin/arm64" || run.KiCad != "10.0.3" || run.Timeout != "6h" || run.CGO != "1" || run.GoEnv != "off" || run.GoWork != "off" || run.GoFlags != "" || run.GoExperiment != "" {
...
func TestPublishedV22Evaluation(t *testing.T) {
	if _, err := os.Stat(filepath.Join(root(), publication)); os.IsNotExist(err) {
		t.Skip("V22 public evaluation not yet published")
	}
	TestV22Freeze(t)
	...
	for i, now := range report.Cases {
		...
		for _, historical := range []capabilitybaselinev10.CaseEvidence{before, v18.Cases[i], v20.Cases[i]} {
	...
	assessment := map[string]any{"schema": "kicadai.public-electrical-assessment.v22", "report_hash": report.Hash, "cases": 24, "replays": replays, "passes": passes, "installed_kicad_promotions": promotions, "additional_complete_passes": additional, "preserved_cases": preserved, "regressions": regressions, "terminal_frontier_before": frontierBefore, "terminal_frontier_after": frontierAfter, "outcome_counts": report.OutcomeCounts, "preservation_passed": preserved == 20 && len(regressions) == 0, "electrical_improvement_passed": len(additional) >= 1 && preserved == 20 && len(regressions) == 0, "experimental": true}
	data, err := json.Marshal(assessment)
	if err != nil {
		t.Fatal(err)
	}
	t.Logf("V22_ASSESSMENT %s", data)
	if len(regressions) != 0 || preserved != 20 {
		t.Error("V22 preservation failed; retain and publish the negative assessment")
	}
}

func TestV22ReportAuthenticationRejectsTampering(t *testing.T) {
	prior := readReport(t, predecessor)
