// Read-only authentication of owned-v4 rehearsal evidence. No language score is
...
import {checkManifest,cleanTerminal,assessAttempt,version as collectorVersion} from './collector.mjs';
import {offlineEnvironment,verifyRuntimeContracts} from './contracts.mjs';

export function authenticateBatch(manifestFile,root) {
  const m=checkManifest(manifestFile),spec={evaluation_id:m.evaluation_id,cases:m.cases},start=read(path.join(root,'start.json')),state=read(path.join(root,'result.json'));
  ...
  assert.equal(state.mode,'offline');assert.equal(start.mode,'offline');assert.equal(start.approval_sha256,null);
  ...
  const bindings={manifest_sha256:hash(manifestFile),result_sha256:hash(path.join(root,'result.json')),selection_sha256:selectionHashes};
  return {spec,state,selections,bindings,acceptance:'offline-only-cannot-establish-live-acceptance'};
}

if(process.argv[1] && path.resolve(process.argv[1])===fileURLToPath(import.meta.url)) {
  try {
    const [manifest,root,...extra]=process.argv.slice(2);assert.ok(manifest && root && extra.length===0,'usage: authenticate.mjs MANIFEST BATCH');
    const {state,acceptance}=authenticateBatch(manifest,root);
    console.log(JSON.stringify({status:'offline-evidence-authenticated',recorded_outcomes:state.recorded_outcomes,acceptance}));
