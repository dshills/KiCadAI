...
The initial test-only schema checker expands local acyclic definitions; it is not a general JSON Schema validator. The later Ajv review adds standards validation, but neither proves that the provider will accept the schema. Existing engineering rules are reused through private bounded decoder helpers; historical public entry points retain their limits. Initial synthetic CLI tests stubbed validation; the subsequent review above reruns complete KiCad validation for all five supported configurations. Both checkpoints are implementing-agent work, not independent semantic or engineering certification, and their package checks are not a whole-repository CI run.
