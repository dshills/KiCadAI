...
func TestVersionFiveContractIsFrozen(t *testing.T) {
	...
	for scanner.Scan() {
		...
		// The manifest records the original bytes of this test. V7 binds the
		// migrated test separately, so its historical self-entry remains
		// evidence rather than a comparison with current bytes.
		if fields[1] != "v5_contract_test.go" && v5FileSHA256(t, filepath.Join(directory, fields[1])) != fields[0] {
			got := v5FileSHA256(t, filepath.Join(directory, fields[1]))
...
func TestVersionFiveImplementationManifestIsFrozen(t *testing.T) {
	directory := v5ContractDirectory(t)
	...
	for _, name := range historicalManifestNames(t, filepath.Join(directory, "V5_IMPLEMENTATION.sha256")) {
		seen, exists := want[name]
		if !exists || seen {
			t.Fatalf("unexpected or duplicate V5 implementation entry %q", name)
		}
		want[name] = true
	}
