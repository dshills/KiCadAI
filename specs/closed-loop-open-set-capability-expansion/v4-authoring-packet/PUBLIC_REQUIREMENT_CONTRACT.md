# Public Open-Topology Requirement Contract for V4 Authoring
...
implementation clue in project text. Manifest-only `v4_case_*` and
`v4_source_*` identities are prohibited here and everywhere else in a
requirement file.
...
assembly behavior; it does not name an internal component or node.
That circuit ID is an assertion-local label, not a reference to a separately
declared object. Reuse it only when assertions intentionally observe the same
whole-assembly quantity.
...
Allowed metrics for V4 authoring:
...
Use `total_harmonic_distortion`; the legacy alias `thd` is not permitted in V4
files even if an older decoder accepts it.
