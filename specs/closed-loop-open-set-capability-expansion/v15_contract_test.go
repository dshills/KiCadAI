...
func TestVersionFourteenGenerationZeroRetirementIsAuthenticatedAndFailClosed(t *testing.T) {
	directory := v7ContractDirectory(t)
	data := v7ReadFile(t, filepath.Join(directory, "V14_GENERATION_ZERO_RETIREMENT.json"))
	...
	if retirement.Schema != "kicadai.closed-loop-open-set-generation-zero-retirement.v14" || retirement.Version != 14 ||
		retirement.Stage != "public_generation_zero" || retirement.RepositoryCommit != "cecefcb9ddffb5068431160b98005af0adacfb05" ||
		retirement.EvaluatorFreezeCommit != retirement.RepositoryCommit || retirement.Reason != "frozen_evaluator_resource_exhaustion" ||
		retirement.FailureClass != "host_memory_pressure_manual_termination" || retirement.TerminalState != "permanently_retired" {
		t.Fatalf("invalid V14 retirement state: %+v", retirement)
	}
	if retirement.RequiredCaseCount != 24 || retirement.CompletedCheckpoints != 2 || retirement.ReplaysPerCase != 2 || retirement.ParallelCaseLimit != 1 ||
		...
		t.Fatal("V14 retirement crossed its incomplete public-only boundary")
	}
	if retirement.ContractManifestSHA256 != v7FileSHA256(t, filepath.Join(directory, "V14_CONTRACT.sha256")) ||
		retirement.EvaluatorManifestSHA256 != v7FileSHA256(t, filepath.Join(directory, "V14_EVALUATOR.sha256")) ||
		retirement.PartialCheckpointsSHA256 != v7FileSHA256(t, filepath.Join(directory, "V14_GENERATION_ZERO_PARTIAL_CHECKPOINTS.sha256")) {
		t.Fatal("V14 retirement historical bindings do not reproduce")
	}
	root := bytes.TrimSuffix(v7ReadFile(t, filepath.Join(directory, "V14_GENERATION_ZERO_ROOT.json")), []byte{'\n'})
	rootDigest := sha256.Sum256(root)
	if hex.EncodeToString(rootDigest[:]) != retirement.EvaluationRootSHA256 {
		t.Fatal("V14 evaluation-root commitment does not reproduce")
	}
	...
	if hex.EncodeToString(digest[:]) != retirement.Hash {
		t.Fatalf("V14 retirement self-hash does not reproduce: got %s want %s", hex.EncodeToString(digest[:]), retirement.Hash)
	}

	pattern := regexp.MustCompile(`^[0-9a-f]{64}  checkpoints/v10_case_00[12]\.json$`)
	scanner := bufio.NewScanner(bytes.NewReader(v7ReadFile(t, filepath.Join(directory, "V14_GENERATION_ZERO_PARTIAL_CHECKPOINTS.sha256"))))
	count := 0
	for scanner.Scan() {
		if !pattern.MatchString(scanner.Text()) {
			t.Fatalf("invalid V14 checkpoint commitment %q", scanner.Text())
		}
		count++
	}
	if err := scanner.Err(); err != nil || count != 2 {
		t.Fatalf("V14 partial checkpoint count=%d error=%v", count, err)
	}
}

func TestVersionFifteenEvaluatorIsFrozenPublicOnlyAndBoundsFailureGraphs(t *testing.T) {
	directory := v7ContractDirectory(t)
	v8VerifyManifest(t, directory, "V15_CONTRACT.sha256")
	data := v7ReadFile(t, filepath.Join(directory, "V15_EVALUATOR_FREEZE.json"))
	var freeze struct {
		Schema                           string `json:"schema"`
		Version                          int    `json:"version"`
		...
		EvaluatorManifestSHA256          string `json:"evaluator_manifest_sha256"`
		V14RetirementSHA256              string `json:"v14_retirement_sha256"`
		CorpusManifestSHA256             string `json:"corpus_manifest_sha256"`
		CorpusChecksumsSHA256            string `json:"corpus_checksums_sha256"`
		...
		V14CheckpointsReused             bool   `json:"v14_checkpoints_reused"`
	...
	if freeze.Schema != "kicadai.closed-loop-open-set-evaluator-freeze.v15" || freeze.Version != 15 ||
		freeze.FreezeParentCommit != "cecefcb9ddffb5068431160b98005af0adacfb05" || freeze.EvaluatorManifest != "V15_EVALUATOR.sha256" ||
		freeze.EvaluatorManifestSHA256 != v7FileSHA256(t, filepath.Join(directory, freeze.EvaluatorManifest)) ||
		freeze.V14RetirementSHA256 != v7FileSHA256(t, filepath.Join(directory, "V14_GENERATION_ZERO_RETIREMENT.json")) {
		t.Fatalf("invalid V15 freeze identity: %+v", freeze)
	}
	if freeze.DiscoveryCaseCount != 24 || freeze.ReplaysPerCase != 2 || freeze.MaximumParallelCases != 1 || freeze.MaximumLiveReplays != 1 ||
		...
		!freeze.ImmutableV10CorpusReused || freeze.V14CheckpointsReused ||
		!freeze.ProductionPath || freeze.HeldOutAccessSurface || freeze.RealCorpusEvaluated || freeze.ExternalKeyOpened {
		t.Fatal("V15 freeze does not enforce its bounded public-only evaluator")
	}
