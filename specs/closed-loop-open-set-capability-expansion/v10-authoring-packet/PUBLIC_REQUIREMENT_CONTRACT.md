# Public Open-Topology Requirement Contract for V10 Authoring
...
namespace. Every number is a finite JSON number. Manifest-only `v10_case_*` and
`v10_source_*` strings are prohibited throughout the requirement.
...
Safety impact is fixed by the supplied assignment and remains assignment and
authorship provenance metadata. It is not a sixth root field and must not be
copied into the authored requirement JSON. In the corpus rules, a requirement's
specified safety impact means this immutable assignment-bound classification,
not an author-selected JSON field.
