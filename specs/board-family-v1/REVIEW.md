...
- Ten numerical cases and three clean deterministic replays passed. Native/config/BOM bytes matched per profile. Relevant generator, validator, electrical and reference source hashes are unchanged and checked before reusing those results.
...
- Final complete bounded repository regression passed in **33.535 s**, with provider keys removed and unchanged Go packages eligible for cache. Focused tests, recorded-response regression and current-source `go vet` for the affected production/replay packages pass. The exhaustive release tier is not claimed.
- The final publication verifies **221 new hashes**, all eighteen new ledger indices, source/binary identities across interrupted runs, original outcome flags, replay links and numerical costs. Together with unchanged original publications, **603 recorded file hashes** reconcile. The original seventeen ledger entries remain unchanged as parsed records in the full snapshot; the original snapshot file itself is preserved byte-for-byte.
