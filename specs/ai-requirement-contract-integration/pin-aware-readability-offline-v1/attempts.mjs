...
const final=attempts.at(-1),dev=attempts.at(-2);
...
const result={schema:'kicadai.pin-aware-bounded-attempts.v1',attempts,development_attempts:3,frozen_final_attempts:1,final_matches_last_development_inputs_and_available_primary_projects:true,provider_calls:0,benchmark_passes_added:0};
...
console.log(JSON.stringify({attempts:attempts.map(a=>({label:a.label,exit_code:a.exit_code,technical_passes:a.cases.filter(c=>c.complete_technical_pass).length})),final_matches_dev3:true}));
