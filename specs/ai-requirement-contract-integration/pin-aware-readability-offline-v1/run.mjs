...
const base='f8b29a9904dd02c38583cb4c15d72f55b5ee6b75',prefix='kicadai-pin-aware-readability-offline-v1';
...
if(/^native-(dev[123]|final)$/.test(mode)){
  ...
  env.KICADAI_PIN_AWARE_REPLAY_ARTIFACTS=root;
  ...
  args=['test','./internal/compositionlowering','-run','^TestFunctionalPinAwareSerializedReplay$','-v','-count=1','-timeout=45m'];timeout=46*60*1000;
...
else if(mode==='race-ownership')args=['test','-race','./internal/compositionlowering','-run','^TestFunctional(Ownership|PinAware)','-short','-count=1','-timeout=12m'];
else if(/^focused(?:-dev[23]|-final)?$/.test(mode))args=['test',...packages,'-run','Test(Functional|NativeBlock)','-count=1','-timeout=3m'];
