...
const phase=dirname(fileURLToPath(import.meta.url)),root=resolve(process.argv[2]||'/tmp/kicadai-pin-aware-readability-offline-v1-final');
...
for(const name of ['standalone_regulator','controller_adc_100ma']){
 ...
 for(const c of components.values()){
  ...
  if(!parent)continue;
  const shared=[];
  for(const n of sch.circuit.nets){if(['ground','no_connect'].includes(n.role)||!n.connect.some(e=>e.slice(0,e.lastIndexOf('.'))===c.id))continue;
   for(const e of n.connect){const i=e.lastIndexOf('.');if(e.slice(0,i)!==parent.id)continue;const pin=e.slice(i+1);shared.push({net:n.name,role:n.role,pin,side:side(parent.ref,pin),priority:n.role==='power'?0:1});}}
