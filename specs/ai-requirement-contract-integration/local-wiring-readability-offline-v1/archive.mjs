...
const names=['dev1','dev2','dev3','final'].map(n=>'kicadai-local-wiring-readability-offline-v1-'+n).filter(n=>existsSync(join('/tmp',n)));
const sourceRoot=join(repo,'.cache/local-wiring-readability-v1-sources');
...
const archive=join(repo,'.cache/local-wiring-readability-offline-v1-all-runs.tar.gz');assert(!existsSync(archive));
