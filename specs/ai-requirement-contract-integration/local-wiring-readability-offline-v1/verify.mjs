...
const root=resolve(process.argv[2]||'/tmp/kicadai-local-wiring-readability-offline-v1-final');
const previous='/tmp/kicadai-joint-annotation-placement-offline-v1-final';
const previousBoard='/tmp/kicadai-joint-annotation-placement-offline-v1-final';
const base='3c1572f7cd9a8120b45e16c42c67500a5fb1097b';
...
for(const name of ['standalone_regulator','controller_adc_100ma']) {
  ...
  assert.equal(req.explicit_circuit.schematic.layout.functional_profile,'ownership-v5');
  // Full request equality except the explicitly versioned drawing layout.
  const physicalRequest=structuredClone(req);
  physicalRequest.explicit_circuit.schematic.layout.functional_profile=prior.explicit_circuit.schematic.layout.functional_profile;
  ...
  const pcbPath=pcbFiles[0].path,pcb=boardAudit(join(dir,'first',pcbPath),req.explicit_circuit.components),oldPCB=boardAudit(join(previousBoard,name,'first',pcbPath),prior.explicit_circuit.components);
...
const snapshot=json(join(repo,'.cache/local-wiring-readability-v1-sources',label+'.json'));
...
const result={schema:'kicadai.local-wiring-readability-offline-verification.v1',base,root,source,cases,inventory:{file_count:raw.length,total_bytes:raw.reduce((n,f)=>n+f.bytes,0),sha256:hash(JSON.stringify(raw))},provider_calls:0,frozen_cases_added:0,technical_gate_passed:cases.every(c=>c.technical_gate_passed),technical_examples_passed:cases.filter(c=>c.technical_gate_passed).length,complete_practical_benchmark_cases:0,planned_examples:2,readability:'separate visual review; native checks do not establish readability'};
