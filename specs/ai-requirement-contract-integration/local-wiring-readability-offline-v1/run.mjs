...
const base='3c1572f7cd9a8120b45e16c42c67500a5fb1097b',prefix='kicadai-local-wiring-readability-offline-v1';
...
if(/^native-(dev[123]|final)$/.test(mode)){
  ...
  env.KICADAI_LOCAL_WIRING_REPLAY_ARTIFACTS=root;
  ...
  args=['test','./internal/compositionlowering','-run','^TestFunctionalLocalWiringSerializedReplay$','-v','-count=1','-timeout=45m'];timeout=46*60*1000;
...
else if(mode==='race-ownership')args=['test','-race','./internal/compositionlowering','-run','^TestFunctional(Ownership|PinAware|Joint|LocalWiring)','-short','-count=1','-timeout=12m'];
else if(/^focused(?:-dev[23]|-final)?$/.test(mode))args=['test',...packages,'-run','Test(Functional|NativeBlock|NativeJoint|NativeLocalWiring)','-count=1','-timeout=3m'];
else if(/^diagnostic-(layout|candidates)-[1-3]$/.test(mode)){env.KICADAI_LOCAL_WIRING_DIAGNOSTIC_INPUT='/tmp/kicadai-joint-annotation-placement-offline-v1-final';env.KICADAI_LOCAL_WIRING_DIAGNOSTIC_TRANSACTION=join(phase,'diagnostic-transaction-'+mode.at(-1)+'.json');args=['test',mode.includes('layout')?'./internal/compositionlowering':'./internal/transactions','-run',mode.includes('layout')?'^TestFunctionalLocalWiringRecordedLayoutDiagnostic$':'^TestNativeLocalWiringFrozenCandidateDiagnostic$','-v','-count=1','-timeout=5m'];}
