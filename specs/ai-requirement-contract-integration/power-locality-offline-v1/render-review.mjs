...
for(const name of ['standalone_regulator','controller_adc_100ma']){
 ...
 for(const group of schematic.layout.groups){
  ...
  const x=Math.max(0,Math.floor(Math.min(...symbols.map(s=>s.at.x_mm))-85)),y=Math.max(0,Math.floor(Math.min(...symbols.map(s=>s.at.y_mm))-85));
  const width=Math.ceil(Math.max(...symbols.map(s=>s.at.x_mm))+85-x),height=Math.ceil(Math.max(...symbols.map(s=>s.at.y_mm))+85-y);
  const out=join(dir,'review-'+group.id+'.png');assert(!existsSync(out));
  crops.push({name,group:group.id,label:group.label,x,y,width,height,margin_mm:85});
