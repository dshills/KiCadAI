...
const base='e6f648bbbb05d1d8bc883e61356511d8708aec2e',prefix='kicadai-power-locality-offline-v1';
...
if(/^native-(dev[123]|final)$/.test(mode)){
  ...
  env.KICADAI_POWER_LOCALITY_REPLAY_ARTIFACTS=root;
  ...
  args=['test','./internal/compositionlowering','-run','^TestFunctionalPowerLocalitySerializedReplay$','-v','-count=1','-timeout=45m'];timeout=46*60*1000;
...
else if(mode==='race-ownership')args=['test','-race','./internal/compositionlowering','-run','^TestFunctional(Ownership|PinAware|Joint|LocalWiring|PowerLocality)','-short','-count=1','-timeout=12m'];
else if(/^focused(?:-dev[23]|-native-dev[123]|-final)?$/.test(mode))args=['test',...packages,'-run','Test(Functional|NativeBlock|NativeJoint|NativeLocalWiring)','-count=1','-timeout=3m'];
else if(/^diagnostic-(layout|candidates)-[1-3]$/.test(mode)){env.KICADAI_LOCAL_WIRING_DIAGNOSTIC_INPUT='/tmp/kicadai-local-wiring-readability-offline-v1-final';env.KICADAI_LOCAL_WIRING_DIAGNOSTIC_TRANSACTION=join(phase,'diagnostic-transaction-'+mode.at(-1)+'.json');args=['test',mode.includes('layout')?'./internal/compositionlowering':'./internal/transactions','-run',mode.includes('layout')?'^TestFunctionalPowerLocalityRecordedLayoutDiagnostic$':'^TestNativeLocalWiringFrozenCandidateDiagnostic$','-v','-count=1','-timeout=5m'];}
