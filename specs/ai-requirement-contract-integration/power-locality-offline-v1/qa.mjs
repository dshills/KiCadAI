...
const finalModes=['native-dev2','lint-dev2','focused-native-dev2','focused-final','native-final','lint-final','race-ownership','race-layout','race-designapi','vet','full'];
const executions=[];
for(const mode of modes){const receipt=json(join(phase,mode+'.execution.json')),snapshot=json(receipt.source_snapshot);
 ...
 if(mode==='full')assert([0,1].includes(receipt.code));else assert.equal(receipt.code,['focused','focused-dev3','native-dev1'].includes(mode)?1:0);
 ...
 if(mode.startsWith('native-')){assert(receipt.args.includes('-timeout=45m'));assert.equal(receipt.code,mode==='native-dev1'?1:0);}
...
const attempts=json(join(phase,'attempts.json'));assert.equal(attempts.development_attempts,2);assert.equal(attempts.frozen_final_attempts,1);assert.equal(attempts.final_matches_last_development_inputs_and_available_primary_projects,true);
assert(!existsSync('/tmp/kicadai-power-locality-offline-v1-dev3'));
...
assert(metrics.cases.every(c=>c.decoupling_non_regression));
const capacitors=metrics.cases.flatMap(c=>c.decoupling);assert.equal(capacitors.filter(c=>c.current_mm<c.previous_mm-1e-8).length,8);assert.equal(capacitors.filter(c=>Math.abs(c.current_mm-c.previous_mm)<=1e-8).length,3);
const rails=json(join(phase,'power-rail-audit.json'));assert.deepEqual(rails.cases.map(c=>[c.previous.visible_capacitor_paths,c.current.visible_capacitor_paths,c.current.eligible_capacitor_paths]),[[0,4,4],[0,4,4]]);
...
const diagnosis=json(join(phase,'diagnosis.json'));assert.equal(diagnosis.diagnostics.length,3);assert(diagnosis.diagnostics.every(d=>d.candidates.exit_code===0));assert.equal(diagnosis.negative_executions.length,3);
...
assert(wiring.cases.every(c=>c.current.wire_length_mm<c.previous.wire_length_mm&&c.current.paper===c.previous.paper));
...
assert.equal(json(join(phase,'history-authentication.json')).previous[0].commit,'e6f648bbbb05d1d8bc883e61356511d8708aec2e');
