...
const root=resolve(process.argv[2]||'/tmp/kicadai-power-locality-offline-v1-final');
const previous='/tmp/kicadai-local-wiring-readability-offline-v1-final';
const previousBoard='/tmp/kicadai-local-wiring-readability-offline-v1-final';
const base='e6f648bbbb05d1d8bc883e61356511d8708aec2e';
...
for(const name of ['standalone_regulator','controller_adc_100ma']) {
  ...
  assert.equal(req.explicit_circuit.schematic.layout.functional_profile,'ownership-v6');
...
const snapshot=json(join(repo,'.cache/power-locality-v1-sources',label+'.json'));
...
const result={schema:'kicadai.power-locality-offline-verification.v1',base,root,source,cases,inventory:{file_count:raw.length,total_bytes:raw.reduce((n,f)=>n+f.bytes,0),sha256:hash(JSON.stringify(raw))},provider_calls:0,frozen_cases_added:0,technical_gate_passed:cases.every(c=>c.technical_gate_passed),technical_examples_passed:cases.filter(c=>c.technical_gate_passed).length,complete_practical_benchmark_cases:0,planned_examples:2,readability:'separate visual review; native checks do not establish readability'};
