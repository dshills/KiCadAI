...
const names=['dev1','dev2','dev3','final'].map(n=>'kicadai-power-locality-offline-v1-'+n).filter(n=>existsSync(join('/tmp',n)));
const sourceRoot=join(repo,'.cache/power-locality-v1-sources');
...
const archive=join(repo,'.cache/power-locality-offline-v1-all-runs.tar.gz');assert(!existsSync(archive));
