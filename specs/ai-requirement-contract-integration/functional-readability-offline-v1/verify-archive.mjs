...
const phase=dirname(fileURLToPath(import.meta.url)),repo=resolve(phase,'../../..'),archive=join(repo,'.cache/functional-readability-offline-v1-all-runs.tar.gz');
...
for(const e of entries){const first=e.path.split('/')[0],root=first==='functional-readability-v1-sources'?join(repo,'.cache'): '/tmp';assert.deepEqual(await digest(join(root,e.path)),{bytes:e.bytes,sha256:e.sha256},'Original evidence changed');}
...
const result={schema:'kicadai.functional-readability-archive.v1',archive,...await digest(archive),manifest_sha256:hash(readFileSync(manifest)),streamed,all_original_files_unchanged:true,archive_policy:'Created once with COPYFILE_DISABLE=1. All regular data/source files are exact; any supported AppleDouble metadata is separately validated. No full extraction created.',provider_calls:0};
