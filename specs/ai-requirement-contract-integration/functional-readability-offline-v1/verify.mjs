...
const root=resolve(process.argv[2]||'/tmp/kicadai-functional-readability-offline-v1-final');
const previous='/tmp/kicadai-pcb-seed-rotated-fields-offline-v1-final';
const base='9ac563194a4a3b89d52d54b8ee38ce04a157ff58';
...
for(const name of ['standalone_regulator','controller_adc_100ma']) {
  ...
  assert.equal(req.explicit_circuit.schematic.layout.functional_profile,'ownership-v1');
...
const snapshot=json(join(repo,'.cache/functional-readability-v1-sources',label+'.json'));
...
const result={schema:'kicadai.functional-readability-offline-verification.v1',base,root,source,cases,inventory:{file_count:raw.length,total_bytes:raw.reduce((n,f)=>n+f.bytes,0),sha256:hash(JSON.stringify(raw))},provider_calls:0,frozen_cases_added:0,technical_gate_passed:true,complete_practical_benchmark_cases:0,planned_examples:2,readability:'separate visual review; native checks do not establish readability'};
