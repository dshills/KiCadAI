...
const root=resolve(process.argv[2]||'/tmp/kicadai-joint-annotation-placement-offline-v1-final');
const previous='/tmp/kicadai-pin-aware-readability-offline-v1-final';
const previousBoard='/tmp/kicadai-ownership-locality-offline-v2-final';
const base='7647dda9ba4d94d57213fe8f7ecaf5d474c667fe';
...
for(const name of ['standalone_regulator','controller_adc_100ma']) {
  ...
  assert.equal(req.explicit_circuit.schematic.layout.functional_profile,'ownership-v4');
...
const snapshot=json(join(repo,'.cache/joint-annotation-placement-v1-sources',label+'.json'));
...
const result={schema:'kicadai.joint-annotation-placement-offline-verification.v1',base,root,source,cases,inventory:{file_count:raw.length,total_bytes:raw.reduce((n,f)=>n+f.bytes,0),sha256:hash(JSON.stringify(raw))},provider_calls:0,frozen_cases_added:0,technical_gate_passed:cases.every(c=>c.technical_gate_passed),technical_examples_passed:cases.filter(c=>c.technical_gate_passed).length,complete_practical_benchmark_cases:0,planned_examples:2,readability:'separate visual review; native checks do not establish readability'};
