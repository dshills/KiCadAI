...
const modes=['diagnostic-frozen','diagnostic-frozen-2','diagnostic-frozen-3','diagnostic-layout-1','diagnostic-candidates-1','focused','focused-dev2','focused-final','native-dev1','native-dev2','native-final','lint','lint-dev2','lint-final','race-ownership','race-layout','race-designapi','vet','full'];
assert.deepEqual(readdirSync(phase).filter(p=>p.endsWith('.execution.json')).map(p=>p.replace('.execution.json','')).sort(),[...modes].sort());
const frozen=json(join(repo,'.cache/joint-annotation-placement-v1-sources/native-final.json'));assert.equal(frozen.files.length,16);
...
const finalModes=['native-dev2','focused-final','native-final','lint-final','race-ownership','race-layout','race-designapi','vet','full'];
const executions=[];
for(const mode of modes){const receipt=json(join(phase,mode+'.execution.json')),snapshot=json(receipt.source_snapshot);
 ...
 if(mode==='full')assert([0,1].includes(receipt.code));else assert.equal(receipt.code,mode==='diagnostic-frozen'?1:0);
...
assert(!existsSync('/tmp/kicadai-joint-annotation-placement-offline-v1-dev3'));
...
const metrics=json(join(phase,'readability-metrics.json'));assert(metrics.cases.every(c=>!c.decoupling_non_regression));assert.equal(metrics.cases.flatMap(c=>c.decoupling).length,11);assert.equal(metrics.practical_benchmark_passes_added,0);
...
const diagnosis=json(join(phase,'diagnosis.json'));assert.equal(diagnosis.single_candidate_conflicts.length,1);assert.equal(diagnosis.native_final_projection_matches_diagnostic,true);
...
assert.equal(json(join(phase,'history-authentication.json')).previous[0].commit,'7647dda9ba4d94d57213fe8f7ecaf5d474c667fe');
