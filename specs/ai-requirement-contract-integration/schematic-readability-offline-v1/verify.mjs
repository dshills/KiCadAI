...
const root=resolve(process.argv[2]||'/tmp/kicadai-schematic-readability-offline-v1-final');
const previous='/tmp/kicadai-explicit-reference-promotion-offline-v1-final';
const base='b9d48213aa0431d5cf19824287c6403a4aea5529';
...
function boardAudit(path,components) {
  ...
  return {normalized,connectedPads,unconnectedPads,paper:children(board,'paper')[0]};
}
...
assert.equal(seal.projects.length,4);
const cases=[];
for(const name of ['standalone_regulator','controller_adc_100ma']) {
  ...
  assert.deepEqual(pcb.normalized,oldPCB.normalized,'PCB geometry/connectivity changed across phases');
  ...
  cases.push({name,components:physical.size,requirements_unchanged:true,electrical_and_pcb_invariants:invariant,runs,native_physical_nets:sch.nets.length,native_physical_endpoints:endpoints,pcb:{connected_pads:pcb.connectedPads,unconnected_pads:pcb.unconnectedPads,geometry_and_connectivity_unchanged:true,comparison_to_previous_phase:'net IDs resolved to names; drawing-paper setting excluded; all other S-expression content exact',previous_paper:oldPCB.paper,current_paper:pcb.paper},project_files:first.length,byte_exact_generated_project_replay:true,normalization:'none',assertions:attempt.assertions});
}
...
const result={schema:'kicadai.schematic-readability-offline-verification.v1',base,root,source,cases,inventory:{file_count:raw.length,total_bytes:raw.reduce((n,f)=>n+f.bytes,0),sha256:hash(JSON.stringify(raw))},provider_calls:0,frozen_cases_added:0,readability:'separate visual review; native checks do not establish readability'};
