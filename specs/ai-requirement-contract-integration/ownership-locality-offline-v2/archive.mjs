...
const names=['dev1','dev2','final'].map(n=>'kicadai-ownership-locality-offline-v2-'+n).filter(n=>existsSync(join('/tmp',n)));
const sourceRoot=join(repo,'.cache/ownership-locality-v2-sources');
...
const archive=join(repo,'.cache/ownership-locality-offline-v2-all-runs.tar.gz');assert(!existsSync(archive));
