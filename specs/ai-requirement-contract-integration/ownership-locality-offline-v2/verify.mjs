...
const root=resolve(process.argv[2]||'/tmp/kicadai-ownership-locality-offline-v2-final');
const previous='/tmp/kicadai-functional-readability-offline-v1-final';
const base='c2ec8c88da11f8cf4aac498775e216ae2baaa34d';
...
for(const name of ['standalone_regulator','controller_adc_100ma']) {
  ...
  assert.equal(req.explicit_circuit.schematic.layout.functional_profile,'ownership-v2');
...
const snapshot=json(join(repo,'.cache/ownership-locality-v2-sources',label+'.json'));
...
const result={schema:'kicadai.ownership-locality-offline-verification.v2',base,root,source,cases,inventory:{file_count:raw.length,total_bytes:raw.reduce((n,f)=>n+f.bytes,0),sha256:hash(JSON.stringify(raw))},provider_calls:0,frozen_cases_added:0,technical_gate_passed:true,complete_practical_benchmark_cases:0,planned_examples:2,readability:'separate visual review; native checks do not establish readability'};
