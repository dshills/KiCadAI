...
const base='c2ec8c88da11f8cf4aac498775e216ae2baaa34d',prefix='kicadai-ownership-locality-offline-v2';
...
if(/^native-(dev[12]|final)$/.test(mode)){
  ...
  env.KICADAI_LOCALITY_REPLAY_ARTIFACTS=root;
  ...
  args=['test','./internal/compositionlowering','-run','^TestFunctionalLocalitySerializedReplay$','-v','-count=1','-timeout=45m'];timeout=46*60*1000;
...
else if(mode==='race-ownership')args=['test','-race','./internal/compositionlowering','-run','^TestFunctionalOwnership','-count=1','-timeout=12m'];
