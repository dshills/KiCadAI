...
const root=resolve(process.argv[2]||'/tmp/kicadai-pcb-seed-rotated-fields-offline-v1-final');
const previous='/tmp/kicadai-schematic-readability-offline-v1-final';
const base='446e19c9b934ad8026b4e9ec2996b47d9ef413f9';
...
for(const name of ['standalone_regulator','controller_adc_100ma']) {
  ...
  assert.equal(req.explicit_circuit.placement_seed.hash,prior.explicit_circuit.generation_hash,'Historical seed not preserved');
...
const label=root.endsWith('-dev1')?'native-dev1':root.endsWith('-dev2')?'native-dev2':'native-final';
const snapshot=json(join(repo,'.cache/pcb-seed-rotated-fields-v1-sources',label+'.json'));
...
const result={schema:'kicadai.pcb-seed-rotated-fields-offline-verification.v1',base,root,source,cases,inventory:{file_count:raw.length,total_bytes:raw.reduce((n,f)=>n+f.bytes,0),sha256:hash(JSON.stringify(raw))},provider_calls:0,frozen_cases_added:0,technical_gate_passed:true,complete_practical_benchmark_cases:0,planned_examples:2,readability:'separate visual review; native checks do not establish readability'};
