...
const phase=dirname(fileURLToPath(import.meta.url)),repo=resolve(phase,'../../..'),archive=join(repo,'.cache/pcb-seed-rotated-fields-offline-v1-all-runs.tar.gz');
...
for(const e of entries){const first=e.path.split('/')[0],root=first==='pcb-seed-rotated-fields-v1-sources'?join(repo,'.cache'): '/tmp';assert.deepEqual(await digest(join(root,e.path)),{bytes:e.bytes,sha256:e.sha256},'Original evidence changed');}
...
const result={schema:'kicadai.pcb-seed-rotated-fields-archive.v1',archive,...await digest(archive),manifest_sha256:hash(readFileSync(manifest)),streamed,all_original_files_unchanged:true,initial_verification:'archive.mjs created the archive successfully; its strict verifier rejected macOS-added AppleDouble metadata. This read-only follow-up validates and inventories that metadata explicitly. No archive was recreated.',provider_calls:0};
