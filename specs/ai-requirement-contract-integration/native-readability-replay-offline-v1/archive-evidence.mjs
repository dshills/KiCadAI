...
const repo=process.cwd(),prefix='kicadai-native-readability-replay-offline-v1-',names=[...Array.from({length:11},(_,i)=>prefix+'dev'+(i+1)).filter(n=>existsSync(join('/tmp',n))),prefix+'final'];
const archive=resolve(repo,'.cache/native-readability-replay-offline-v1-all-runs.tar.gz'),extracted=resolve(repo,'.cache/native-readability-replay-offline-v1-archive-verification');
...
const snapshots='native-readability-replay-v1-sources',sourceInventory=await inventory(join(repo,'.cache',snapshots),true);
run(['-czf',archive,'-C','/tmp',...names,'-C',join(repo,'.cache'),snapshots]);mkdirSync(extracted);run(['-xzf',archive,'-C',extracted]);
...
assert.deepEqual(await inventory(join(extracted,snapshots),false),sourceInventory);
console.log(JSON.stringify({verified_utc:new Date().toISOString(),archive,archive_bytes:statSync(archive).size,archive_sha256:(await fileDigest(archive,false)).sha256,extracted,source_snapshots:sourceInventory,compile_only_attempts_without_raw_directory:['dev7'],all_development_and_final_runs_retained:true,fresh_extraction_verified:true,exact_existing_secret_absent:true,inventories,commands,provider_calls:0},null,2));
