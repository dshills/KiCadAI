...
const root=resolve(process.argv[2]||'/tmp/kicadai-native-readability-replay-offline-v1-final');
const previous='/tmp/kicadai-schematic-readability-offline-v1-final';
const base='ec2ececb4e52643f0767d0fcbb70213f1ec7209c';
...
assert.equal(seal.projects.length,2);
const cases=[];
for(const name of ['standalone_regulator','controller_adc_100ma']) {
  ...
  if(name==='controller_adc_100ma') {
    const r=json(join(dir,'first_workflow.json'));
    assert.equal(new Set(r.stages.map(s=>s.name)).size,r.stages.length);
    const stage=n=>r.stages.find(s=>s.name===n);
    for(const n of stages.slice(0,4))assert.equal(stage(n)?.status,'ok');
    assert.equal(stage('project_write')?.status,'blocked');
    for(const n of ['writer_correctness','validation','kicad_checks'])assert.equal(stage(n)?.status,'skipped');
    assert.equal(stage('simulation'),undefined);
    assert(stage('project_write').issues.some(i=>i.message==='native annotation profile does not support rotated field C1.property.7'));
    assert(!existsSync(join(dir,'second_workflow.json')));
    assert(!existsSync(join(dir,'first'))&&!existsSync(join(dir,'second')));
    assert(!seal.projects.some(p=>p.name===name));
    cases.push({name,requirements_unchanged:true,electrical_and_pcb_intent_unchanged:true,resolution_provenance:{previous:previousResolution[0],current:currentResolution[0],cross_phase_exclusion:'only KiCadAI Resolution Hash; all other schematic circuit fields exact'},promotion:'pass',final_native_gate:'failed',blocking_stage:'project_write',reason:'native annotation profile does not support rotated field C1.property.7',native_projects:0,serialized_project_replay:'not reached',assertions:attempt.assertions});
    continue;
  }
  ...
  // A successful authentication is not a successful phase gate. Retain the
  // observed preservation failure; never normalize away placements/routes.
  assert.equal(pcbUnchanged,false,'Recorded negative board-preservation evidence changed');
  assert(changedPlacements.length>0&&pcb.routes_sha256!==oldPCB.routes_sha256);
...
const snapshot=json(join(repo,'.cache/native-readability-replay-v1-sources/final.json'));
...
const result={schema:'kicadai.native-readability-replay-offline-verification.v1',base,root,source,cases,inventory:{file_count:raw.length,total_bytes:raw.reduce((n,f)=>n+f.bytes,0),sha256:hash(JSON.stringify(raw))},provider_calls:0,frozen_cases_added:0,phase_gate_passed:false,complete_phase_examples:0,planned_examples:2,readability:'separate visual review; native checks do not establish readability'};
const receipt=join(phase,'verification.json');
