...
// Invoke with: node run-campaign-v2.mjs <binary> <root> <campaign-root> <phase>
...
if (!binaryArg || !rootArg || !campaignRootArg || !['baseline','final','paired'].includes(phase) || process.argv.length!==6) {
  throw new Error('usage: node run-campaign-v2.mjs <binary> <root> <campaign-root> <baseline|final|paired>');
}
...
const corpus=readJSON(path.join(spec,'corpus.json')), freeze=readJSON(path.join(spec,'freeze-v2.json'));
...
const required=[...git('ls-files','internal/practicalboardeval','cmd/practical-board-eval').split('\n'),...['SPEC.md','SCORING.md','FEASIBILITY.md','PLAN.md','PREPARATION.md','QUALITY.md','corpus.json','environment.json','run-campaign.mjs','evidence-utils.mjs','evidence-utils.test.mjs','run-campaign-v2.mjs','protocol-v2.test.mjs','PROTOCOL-V2.md','protocol-v2-authorization.json','freeze.json'].map(p=>'specs/practical-sensor-controller-boards/'+p)];
...
save(path.join(output,'campaign-start.json'),{
  protocol:'v2',provider_request_timeout_seconds:300,
  ...
  binary_sha256:sha(binaryBytes),freeze_sha256:sha(fs.readFileSync(path.join(spec,'freeze-v2.json'))),
...
for(const item of inputs) {
  ...
  const args=['--mode','case','--freeze',path.join(spec,'freeze-v2.json'),'--root',root,'--output',caseDir,'--case',item.id,'--campaign',phase,'--journal',journal,'--kicad-cli',cli];
