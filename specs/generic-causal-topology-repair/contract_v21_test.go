...
type v21EvaluatorFreeze struct {
	...
	MaintenanceRevision              int      `json:"maintenance_revision"`
	GenerationZeroManifestSHA256     string   `json:"generation_zero_evaluator_manifest_sha256"`
...
func TestV21ContractAndEvaluatorManifests(t *testing.T) {
	...
	if freeze.Schema != "kicadai.closed-loop-open-set-evaluator-freeze.v21" || freeze.Version != 21 || freeze.FreezeParentCommit != "c31f3bc7af70c2787ddb728fb65358a207f71ded" {
		t.Fatalf("invalid V21 identity freeze: %+v", freeze)
	}
	if freeze.MaintenanceRevision != 1 || freeze.GenerationZeroManifestSHA256 != "400b3ab32100b356d3db373c3111ce04ee678785fcc8598d93e20100c532320b" || freeze.EvaluationStatus != "not_run_after_maintenance" {
		t.Fatalf("maintenance must retain historical attribution without claiming a new evaluation: %+v", freeze)
	}
